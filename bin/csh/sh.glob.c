...
static	char	*Sccsid = "@(#)$Header: /d2/3.7/src/bin/csh/RCS/sh.glob.c,v 1.1 89/03/27 14:53:22 root Exp $";
/*
 * $Log:	sh.glob.c,v $
 * Revision 1.1  89/03/27  14:53:22  root
 * Initial check-in for 3.7
 * 
 * Revision 1.4  87/02/16  17:26:13  vjs
 * quiet void signal(2) complaints
 * 
 * Revision 1.3  86/07/24  17:12:05  paulm
 * Changes for release 3.5: use opendir/readdir for accessing directories.
 * 
 * Revision 1.2  85/01/16  23:39:10  bob
 * Boosted buffer sizes and other 4.2bsd enhancements & fixes.
 * 
 */
#include "sh.h"
	...
	char *pattern;
{
	...
	dirf = opendir(*gpath==0 ? "." : gpath);
	if (dirf == 0) {
		if (globbed)
			return;
		goto patherr;
	}
	if (fstat(dirf->dd_fd, &stb) < 0)
		goto patherr;
	if (!isdir(stb)) {
		errno = ENOTDIR;
		goto patherr;
...
/*
 * Copy a name from a directory entry structure returned by
 * the 'readdir' library routine into a string buffer, guaranteeing
...
{
	register int cnt;
	register char *from = dentp->d_name;

	cnt = dentp->d_reclen - ((int)from - (int)dentp);
	if (cnt > MAXNAMLEN)
		cnt = MAXNAMLEN;
