...
/* $Source: /d2/3.7/src/bin/csh/RCS/sh.lex.c,v $ */
static	char	*Sccsid = "@(#)$Revision: 1.1 $";
/* $Date: 89/03/27 14:53:26 $ */
#include "sh.h"
...
top:
	...
	do {
		...
		if (c < 0) {
			if (wanteof)
				return (-1);
			if (adrof("ignoreeof") && isatty(SHIN)) {
				if (loginsh)
...
bgetc()
{
	...
	if (cantell) {
		...
		c = fbuf[0][fseekp - fbobp] & TRIM;	
		/* TRIM to keep an 8-bit char from causing damage */
...
again:
	...
	if (buf >= fblocks) {
		register u_char **nfbuf = (u_char **) calloc(fblocks+2, sizeof (u_char **));
		...
		fbuf = nfbuf;
		fbuf[fblocks] = (u_char *) calloc(BUFSIZ, sizeof (char));
		fblocks++;
		goto again;
	}
	if (fseekp >= feobp) {
	...
	c = fbuf[buf][(int) fseekp % BUFSIZ] & TRIM;
	/* TRIM to keep an 8-bit char from causing damage */
	fseekp++;
	return (c);
}
...
settell()
{
	...
	if (lseek(SHIN, 0l, 1) < 0 || errno == ESPIPE)
		return;
	fbuf = (u_char **) calloc(2, sizeof (char **));
	fblocks = 1;
	fbuf[0] = (u_char *)calloc(BUFSIZ, sizeof (char));
