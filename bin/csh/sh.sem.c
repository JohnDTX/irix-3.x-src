...
static	char	*Sccsid = "@(#)$Header: /d2/3.7/src/bin/csh/RCS/sh.sem.c,v 1.1 89/03/27 14:53:32 root Exp $";
	...
	case TPAR:
		...
		if (t->t_dtyp == TCOM && isbfunc(t->t_dcom[0])) {
			...
			if (pid == 0) {
				...
				/*
				 * If func failed then we assume it was
				 * "time" or "nice" and clear FPAR so we can
				 * fork below.  This shouldn't hurt the other
				 * cases where func fails.
				 */
				flags &= ~FPAR;
			}
		}
