...
#ifdef	SVR3
/*
 * System 5.3 returns times() values in HZ's
 */
ptimes(sec, bef, aft)
	time_t sec;
	register struct tbuffer *bef, *aft;
{
	printf("%d.%du %d.%ds ",
		       (aft->cut - bef->cut) / HZ,
		       ((aft->cut - bef->cut) % HZ) * 10 / HZ,
		       (aft->cst - bef->cst) / HZ,
		       ((aft->cst - bef->cst) % HZ) * 10 / HZ);
	psecs(sec);
	printf(" %d%%\n",
		 ((100 * secs(bef, aft)) / (HZ * (sec ? sec : 1))));
}

#else
/*
 * IRIS time values are normalized to 60th's, so this code can run unchanged.
 */
ptimes(sec, bef, aft)
	time_t sec;
	register struct tbuffer *bef, *aft;
