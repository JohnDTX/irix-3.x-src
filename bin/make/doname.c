...
TIMETYPE *tval;
{
	...
	LINEBLOCK lp1, lp2;
#ifdef	sgi
	char sourcename[500],prefix[500],temp[500],concsuff[500];
#else	/* sgi */
 ...
 */
	...
	for(lp = p->linep ; lp!=0 ; lp = lp->nextline)
	{
		...
		for(q = lp->depp ; q!=0 ; q=q->nextdep)
		{
			...
			if(ptime < td1)
#ifdef	sgi
				appendq(&qchain, alias(q->depname));
#else	/* sgi */
