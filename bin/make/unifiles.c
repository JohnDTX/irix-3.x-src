...
register CHARSTAR filename;
{
	...
	*q = LPAREN;
	...
	while(getarch())
	{
		...
		{
			getobj();
			nsym = objhead.ssize / sizeof(objentry);
			for(i = 0; i<nsym ; ++i)
			{
...
getobj()
{
	...
	if( objhead.fmagic != A_MAGIC1 &&
	    objhead.fmagic != A_MAGIC2 )
			fatal1("%s is not an object module", arhead.ar_name);
	skip = objhead.tsize + objhead.dsize;
#ifdef m68000
	skip += objhead.rtsize + objhead.rdsize;
#else
#ifdef vax
