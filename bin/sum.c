...
#define BLOCKSIZE	1024		/* size for block count of file */
...
char **argv;
{
	...
	do {
		...
		if (alg == 1)
			(void) printf("%.5u%6ld", sum, (nbytes+BLOCKSIZE-1)/BLOCKSIZE);
		else {
			tempa.lg = (suma.hl.lo & WDMSK) + (suma.hl.hi & WDMSK);
			lsavhi = (unsigned) tempa.hl.hi;
			lsavlo = (unsigned) tempa.hl.lo;
			(void) printf("%u %ld", (unsigned)(lsavhi + lsavlo), (nbytes+BLOCKSIZE-1)/BLOCKSIZE);
		}
		if(argc > 1)
