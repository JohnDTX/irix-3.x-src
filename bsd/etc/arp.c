	...
	char *kernel, *mem;
{
	...
	if (nl[X_KERNEL_MAGIC].n_type == 0
	    || nl[X_END].n_type == 0
	    || 0 > lseek(mf, (long)nl[X_KERNEL_MAGIC].n_value & 0x7fffffff, 0)
	    || sizeof(kern_end) != read(mf, &kern_end, sizeof(kern_end))
	    || kern_end != nl[X_END].n_value) {
		fprintf(stderr, "arp: %s: namelist wrong\n", kernel);
		exit(1);
	}
	lseek(mf, (long)nl[X_ARPTAB_SIZE].n_value & 0x7fffffff, 0);
#else
...
#if defined(sgi) && defined(SVR3)
	lseek(mf, (long)nl[X_ARPTAB].n_value & 0x7fffffff, 0);
#else
