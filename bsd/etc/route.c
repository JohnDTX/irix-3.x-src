...
flushroutes()
{
	...
	nlist("/unix", nl);
	if (nl[N_KERNEL_MAGIC].n_value == 0) {
		printf("route: \"kernel_magic\", symbol not in namelist\n");
		exit(1);
	}
	if (nl[N_END].n_value == 0) {
		printf("route: \"end\", symbol not in namelist\n");
		exit(1);
	}
#else
	nlist("/vmunix", nl);
	...
	if (nl[N_KERNEL_MAGIC].n_type == 0
		...
		fprintf(stderr, "route: %s: namelist wrong\n", "/unix");
		exit(1);
	}
	lseek(kmem, nl[N_RTHASHSIZE].n_value & 0x7fffffff, 0);
#else
...
#if defined(sgi) & defined(SVR3)
	lseek(kmem, nl[N_RTHOST].n_value & 0x7fffffff, 0);
#else
...
again:
	...
	if (doinghost) {
#if defined(sgi) && defined(SVR3)
		lseek(kmem, nl[N_RTNET].n_value & 0x7fffffff, 0);
#else
