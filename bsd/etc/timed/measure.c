...
#ifdef sgi
#undef RANGE
#define RANGE ((1000-1)/HZ)	/* <<this>> is best expected round trip */
#define PROCESSING_TIME	0 	/* seems silly */
#else
...
empty:
	...
	if (trace) {
		fprintf(fd, "measured delta=%d to %s with %d trials\n",
			measure_delta, inet_ntoa(addr->sin_addr), trials+1);
	}
#endif
	return(status);
}
