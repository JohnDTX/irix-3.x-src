...
char passwd_faked = 0;
char passwd_fake_ok = 1;
static char *
local_fgets(buf,bufsize,file)
char *buf;
int bufsize;
FILE *file;
{
	...
	if (p = fgets(buf,bufsize,file))
		return p;

	if (!passwd_faked && passwd_fake_ok) {
		passwd_faked = 1;
		strncpy(buf,"+",bufsize);
		return buf;
	}
	...
	char *name;
{
	...
	if (!pwf)
		return NULL;
	while (local_fgets(line, BUFSIZ, pwf) != NULL) {
...
getpwent()
{
	...
	if (pwf == NULL && (passwd_faked=0, pwf = fopen(PASSWD, "r")) == NULL) {
		return (NULL); 
	}
	...
	for (;;) {
		if (yp) {
			...
			endnetgrent();
			if (local_fgets(line1, BUFSIZ, pwf) == NULL)  {
				return(NULL);
			}
