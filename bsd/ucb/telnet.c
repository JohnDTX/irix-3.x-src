...
register int f;
{
	...
	case 0:
		onoff = 0;
		bcopy(otc.c_cc, tc.c_cc, sizeof(tc.c_cc));
		break;
	...
	case 2:		/* remote character processing, local echo */
		...
		/*
		 * If user has not specified one way or the other,
	...
	ioctl(fileno(stdin), TCSETA, (char*)&tc);
...
#ifdef sgi
	    if (name) {
		    if (!strcmp(name, "wsiris"))
			    name = "rwsiris";
		    else if (!strcmp(name, "iris-ansi"))
			    name = "iris-ansi-net";
	    }
#endif
...
struct togglelist Togglelist[] = {
    ...
    { "flow",
 	"toggle flow control (XON/XOFF)",
	    0,
		1,
		    &flow,
			"use local flow control (XON/XOFF)" },
    { "localchars",
	...
	va_dcl
{
	...
	va_start(args);
	for (argc = 0, argp = &argv[0];
	     (*argp = va_arg(args, char*)) != 0
			&& argc < sizeof(argv)/sizeof(argv[0]);
	     argc++, argp++)
		;
	va_end(args);
	*argp = 0;
	return (*routine)(argc, &argv[0]);
}
#else
/*
 * Call routine with argc, argv set from args (terminated by 0).
	...
	char *argv[];
{
	...
	NetTrace = stdout;
#ifdef	sgi
	(void)ioctl(0, TCGETA, (char *)&otc);
	autoflush = !(otc.c_lflag & NOFLSH);
	flow = (otc.c_iflag & IXON);
	ntc = otc;
#else
	ioctl(0, TIOCGETP, (char *)&ottyb);
	ioctl(0, TIOCGETC, (char *)&otc);
