...
#endif
#if defined(sgi) || defined(sgi_vax)
	if (NULL == rhs.dptr		/* on failure try lowercase */
	    && lhs.dsize < MAXNAME) {	/* provided length is reasonable */
		char lc[MAXNAME];
		strcpy(lc,name);
		makelower(lc);
		lhs.dptr = lc;
	...
	bool init;
{
	...
	register STAB *s;
	char line[BUFSIZ];
#if defined(sgi) && defined(FLOCK) && defined(DBM)
	static struct flock flck = {F_WRLCK};
#endif
...
#ifdef sgi
	if (fcntl(fileno(af), F_SETLK, (int)&flck) == -1
	    && (errno == EACCES || errno == ENOLCK
		|| errno == EDEADLK || errno == EAGAIN))
#else
...
#ifdef sgi
			while (-1 == fcntl(fileno(af), F_SETLKW, (int)&flck))
 				continue;
#else
...
# ifdef DBM
	if (init)
	{
		...
		DATUM key;
#ifdef sgi
		ypalias_init(af);	/* and add the Yellow Page keys */
#endif sgi
