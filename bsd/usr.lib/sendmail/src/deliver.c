	...
	ENVELOPE *e;
{
	...
	*/
	...
	else
	{
		Errors++;
		usrerr(statmsg);
#if defined(sgi) || defined(vax_sgi)
		syslog(LOG_INFO, "mailer error, mailer=%s error=%d\n",
				 m->m_name, stat);
#endif
	}
...
# ifdef LOG
#ifdef sgi				/* avoid null pointers */
	syslog(LOG_INFO, "%s: to=%s, delay=%s, stat=%s",
	       CurEnv->e_id ? CurEnv->e_id : "(?)",
	       CurEnv->e_to ? CurEnv->e_to : "(?)",
