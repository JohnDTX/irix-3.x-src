/*
 * $Source: /d2/3.7/src/diag/gf/src/RCS/tod.c,v $
 * $Revision: 1.1 $
 * $Date: 89/03/27 15:23:22 $
 */
#include	"../h/types.h"
#include	"../h/systm.h"
#include	"../machine/cpureg.h"
#include	"../machine/tod.h"
...
**  If the chip has been determined to have valid time (VRT bit)
...
time_t	oldtime;
{
	...
	if ( todstat & TD_VALID )
		time = todgettim();	/* time is valid, get it from chip */
	else
		time = oldtime;		/* sorry charlie!		   */
}
...
todinit()
{
	...
	READFIELD( td_regD, tod_dev.td_regD )
	if ( tod_dev.td_regD & RD_VRT )
		todstat = TD_VALID;
	else
		todstat = TD_INVALID;

	READFIELD( td_sgiun.td_sginfo.tds_btflg, tod_dev.td_sgiun.td_sginfo.tds_btflg )
	READFIELD( td_sgiun.td_sginfo.tds_pwrflg, tod_dev.td_sgiun.td_sginfo.tds_pwrflg )
	READFIELD( td_sgiun.td_sginfo.tds_wtimflg, tod_dev.td_sgiun.td_sginfo.tds_wtimflg )
	...
	val;
{
	...
	{
	   case TD_BTFLG:
		READFIELD( td_sgiun.td_sginfo.tds_btflg, val1 )
		WRITEFIELD( td_sgiun.td_sginfo.tds_btflg, val )
		break;

	   case TD_PWRFLG:
		READFIELD( td_sgiun.td_sginfo.tds_pwrflg, val1 )
		WRITEFIELD( td_sgiun.td_sginfo.tds_pwrflg, val )
		break;

	   case TD_WTIMFLG:
		READFIELD( td_sgiun.td_sginfo.tds_wtimflg, val1 )
		WRITEFIELD( td_sgiun.td_sginfo.tds_wtimflg, val )
		break;
