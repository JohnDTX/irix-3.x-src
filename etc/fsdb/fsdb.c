...
static	char	*Sccsid = "@(#)$Header: /d2/3.7/src/etc/fsdb/RCS/fsdb.c,v 1.1 89/03/27 15:39:35 root Exp $";
/*
 * $Log:	fsdb.c,v $
 * Revision 1.1  89/03/27  15:39:35  root
 * Initial check-in for 3.7
 * 
 * Revision 1.2  85/04/29  19:40:53  bob
 * 1. Added symbolic links.
 * 2. Fixed to operate even if superblock does not say that it is 1k fs since
 *    there are no 512b fs & we do not supply fsdb1b.
 * 
 */
...
#define S	sblock.fs
	...
	default:
		...
		exit(1);
#else	notdef
		printf("%s(%s): IS NOT A 1K BYTE BLOCK FILE SYSTEM\nFIX SUPERBLOCK FIRST!!!\n", argv[1], S.s_fname);
#endif	notdef
	}
#endif	FsTYPE
#endif	STANDALONE
