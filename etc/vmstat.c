...
dotimes()
{
	...
	printf("average: %d usec / reclaim\n",
		s.rectime / safequotient(sum.v_pgrec));
	printf("\n");
	printf("%d page ins, %d total time (msec)\n",sum.v_pgin, s.pgintime/10);
	printf("average: %8.1f msec / page in\n",
		s.pgintime / (safequotient(sum.v_pgin)*10.0));
}
...
dosum()
{
	...
	printf("%9d total reclaims (%d%% fast)\n", sum.v_pgrec,
	    (sum.v_fastpgrec * 100) / safequotient(sum.v_pgrec));
...
#else
	printf("%9d text pages saved between execs\n", sum.v_xsfrec);
	printf("%9d text pages recovered\n", sum.v_xifrec);
#endif
...
doforkst()
{
	...
	printf("%d forks, %d pages, average=%.2f\n",
		...
		(float) forkstat.sizfork / safequotient(forkstat.cntfork));
	printf("%d vforks, %d pages, average=%.2f\n",
		forkstat.cntvfork, forkstat.sizvfork,
		(float)forkstat.sizvfork / safequotient(forkstat.cntvfork));
}
