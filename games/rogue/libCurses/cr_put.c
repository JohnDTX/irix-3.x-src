static	char	*Cr_put_c	= "@(#)cr_put.c	1.9";
# include	"curses.ext"
# include	"cr_ex.h"
...
*/
	...
	while (outcol < destcol) {
/* Too tricky with alternate char sets & color! */
		if (_win != NULL
#ifdef	COLOR
			/*
			 * can't just send chars on color terminal
		  ...
		  && !CT
			...
			if (plodflg)	/* avoid a complex calculation */
				...
				c = _win->_y[outline-_win->_begy]
				  [outcol-_win->_begx];
				if ((c&_STANDOUT) == (curscr->_flags&_STANDOUT)
				 && !_mapchr[c] &&  curscr->_charset == CHRNORM
#ifdef	COLOR
				  && _win->_Y[outline-_win->_begy]
				  [outcol-_win->_begx] == _win->_attrib
#endif	COLOR
				)
					putchar(c);
				else
...
#ifdef	BSD
		write(1, &chr, 1);	/* Something is wrong, this works! */
#else
		putchar(chr);
#endif	BSD
	}
...
/*
 * goto [y,x] position (cursor addressing) for iris
 */
irisgoto()
{
	icursor(destcol,destline);
	outcol = destcol;
	outline = destline;
}

/*
 * flush file - also do gflush()
 */
irisfflush(file)
FILE	*file;
{
	fflush(file);
					/*
					 * the if (_iris) is still needed here
					 * in case we aren't on an IRIS or in
					 * case we are called before ginit() or
					 * after gexit()
					 */
	if (_iris)
		gflush();
...
double	y;
{
	...
	j = y + .001;
	y -= j;
	y = _linblit[j+1] + (_linblit[j+2]-_linblit[j+1])*y;
	cmov2((Coord)(x*CWIDTH),(Coord)y);
}
#endif
