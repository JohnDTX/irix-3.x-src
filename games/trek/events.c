	...
	while (1)
	{
		...
		switch (e->evcode)
		{
		  ...
		  case E_SNAP:
			...
			scp = Snapshot;
			scp = bmove(&Status.bases, scp, 2);
			scp = bmove(&Status.date, scp, 12);
			scp = bmove(Quad, scp, sizeof Quad);
			scp = bmove(Event, scp, sizeof Event);
			scp = bmove(Base, scp, sizeof Base);
			scp = bmove(&Etc, scp, sizeof Etc);
			Game.snap = 1;
			break;
			}
