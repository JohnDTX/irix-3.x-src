...
double	d;
{
	...
	if (percent < 90)
	{
		...
		if (percent < 35 || !Game.snap)
		{
			...
			time = Status.date;
			scp=bmove(scp=Snapshot, &Status.bases, 2);
			scp=bmove(scp, &Status.date, 12);
			scp=bmove(scp, Quad, sizeof Quad);
			scp=bmove(scp, Event, sizeof Event);
			scp=bmove(scp, Base, sizeof Base);
			scp=bmove(scp, &Etc, sizeof Etc);
