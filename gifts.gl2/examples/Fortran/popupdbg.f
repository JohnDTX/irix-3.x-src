...
200   CONTINUE
      ...
      print *,'checkpoint 2m, COMAND = ', comand
        CALL CURSOF()
        CALL COLOR(0)
        CALL CLEAR()
        CALL COLOR(1)
...
C
      ...
      CALL QREAD(Y)
      QT = QTEST()
      print *,'checkpoint 2s, DUMMY = ',dummy,', X = ', X, ', Y = ', Y
      IF (QT.NE.0) THEN
        CALL QREAD(DUMMY)
        X = Y
        Y = DUMMY
...
3000  CONTINUE
      j=j+1
      print *,'checkpoint 3s, j = ', j
        IX = GETVAL(266)
        IY = GETVAL(267)
        X = IX
        Y = IY
      print *,'checkpoint 4s, X = ', IX, ', Y = ', IY
C
C         if the cursor is inside the menu
     ...
     2.MTOP)) THEN
          HIGHLT = (MTOP - Y)/16
          CALL CURSOF()
      print *, 'HIGHLT = ', highlt, ', LASTHL = ', lasthl
C
...
C
        QT = QTEST()
        print *, 'checkpoint 6s, QT = ', qt
        IF (QT.NE.0) THEN
          CALL QREAD(DUMMY)
          CALL QREAD(X)
          CALL QREAD(Y)
      print *,'checkpoint 7s, DUMMY = ',dummy,', X = ', X, ', Y = ', Y
          CALL COLOR(0)
          CALL CURSOF()
          CALL RECTFI(MLEFT,MBOTOM,MRIGHT,MTOP)
          CALL CURSON()
