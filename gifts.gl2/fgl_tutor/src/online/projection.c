...
{
#ifdef FORTRAN
    projmenu = defpup(" Projection transforms %t| CALL ORTHO %f| CALL WINDOW %f| CALL PERSPE %f", p1, p2, p3);
    viewmenu = defpup(" Viewing transforms %t| CALL POLARV %f| CALL LOOKAT %f", v1, v2);
#else
 ...
 */
{
    ...
    doublebuffer();
    gconfig();
    helpw = init_help("Projection -- INFORMATION");
    winattach(backw);
    statw = init_state();
    consw = init_cons();
    frontw = init_front();
