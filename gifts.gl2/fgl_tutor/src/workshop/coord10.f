C  coord10.f -- This program introduces projection		
C  transformations.   The ortho() call in drawimage() fits	
C  the 425.0 x 425.0 field into the opened window. The	        
C  field will shrink or stretch to fit the window. This	
C  makes the prefsi call unnecessary.              	
C  Some adjustments to the moveball routine were made        
C  so that the mouse position corresponds to the location      
C  on the field.  						
C  graphics library calls introduced:  ortho, keepas	
...
C  Using the ortho call has made the prefsi call unnecessary     
C  so it has been removed.  The keepas call fixes the shape	
C  of the opened window to a square.					
...
C  An ortho command has been added to drawimage.  The left,	
C  right, top and bottom parameters for ortho specify what the		
C  section of the world is displayed in the opened window.  The	
C  z values are set such that the ball doesn't get clipped no matter	
C  how close or far away the ball is from us. Specifically the z       
C  clipping planes have been set to -10000.0 units behind us and       
C  10000.0 units in front of us.                                       
...
C  moveball is substantially different from the last example.	
C  We can no longer pick an absolute (x, y) location on the screen	
C  with the mouse.  We can pick a location on the window and		
C  calculate the relative distance travelled, depending on the size	
C  of the window.  Note that rdistx and rdisty are the relative 	
C  distance:  the absolute distance * 425.0 (world space size of	
C  field) / screen size of window					
