   ...
   short externflag;
{
   ...
   if ((np = _slookup(name)) == NULL)
      {
	if (externflag) {
		printf("'external' scratchsymbol %s not previously defined\n",
				name);
		fprintf(symf,"'external' symbol %s not previously defined\n",
				name);
		printstate();
	}
	...
	if (!externflag) {
	    printf("scratch symbol %s redefined\n",name);  /*  must be NEW */
	    fprintf(symf,"scratch symbol %s redefined\n",name);
	    printstate();
	}
    ...
    struct sortlist *pp;
{
	...
	for (sp = pp,i = 0; sp != NULL; sp = sp->next) {
		...
		fprintf(symf,"%03x %s",sp->kp->val,sp->kp->sym);
		if ((i++ % 3)==2) putc('\n',symf);
		else {
			putc('\t',symf);
			if ((len=strlen(sp->kp->sym)) < 4) putc('\t',symf);
			if (len < 12) putc('\t',symf);
		}
	}
}
