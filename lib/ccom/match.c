...
# endif
	...
	for( i=0; i<DSIZE; ++i ){
		if( dope[i] ){ /* there is an op... */
			...
			opptr[i] = q;
			if (xdebug) printf("opptr #%d initialized to %o\n",
						i,q->op);
			}
		}
	}
...
match( p, cookie ) NODE *p; {
	...
	if (xdebug) 
	{	printf("match called with node %o, op %d, cookie = ",p,p->in.op);
		prcook(cookie);
		if (cookie==FORREW) printf(" rwtable invoked.\n");
		else printf(" table examination begun at op %d.\n",p->in.op);
	}
# ifndef BUG4
	if( odebug ){
		printf( "match( %o, ", p );
...
expand( p, cookie, cp ) NODE *p;  register char *cp; {
	...
	for( ; *cp; ++cp ){
		...
		case 'Z':  /* special machine dependent operations */
			/* GB pass the pointer itself to zzzcode.  
			   let it gobble what it will */
			cp = zzzcode( p, cp );
			continue;
