...
optim(p) register NODE *p; {
	...
	case SCONV:
/****   BUG (#37) GB SGI  catch SCONVs of function results to 
    	VOID  *****/
		if ((p->in.type==UNDEF)
/*			((p->in.left->in.op==CALL)||
	...
	case ER:
		...
		if(o == PLUS && LO(p) == MINUS && RCON(p) && RCON(p->in.left) &&
 		  conval(p->in.right, MINUS, p->in.left->in.right, p->in.right, p->in.left->in.right )){
			zapleft:
			RO(p->in.left) = FREE;
			LO(p) = FREE;
			p->in.left = p->in.left->in.left;
		}
		if( RCON(p) && LO(p)==o && RCON(p->in.left) && conval( p->in.right, o, p->in.left->in.right, p->in.right, p->in.left->in.right ) ){
			goto zapleft;
			}
		else if( LCON(p) && RCON(p) && conval( p->in.left, o, p->in.right, p->in.left, p->in.right ) ){
			zapright:
			RO(p) = FREE;
