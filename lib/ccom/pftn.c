...
# endif
	...
	/* GB bug #75 interface to fixclass changed to take the node */
	class = fixclass( class, q );
	type = q->in.type;
...
# endif
	...
	if( stp == UNDEF|| stp == FARG ){
		...
		default:
			if(!(class&FIELD)) uerror( "declared argument %.32s is missing", p->sname );
		case MOS:
		case STNAME:
		case MOU:
		case UNAME:
	...
	dsym = p->dimoff;
	ddef = q->fn.cdim;
	/* GB BUG88 - stabs not generated previously for arrays whose
	   length has not been declared.  If this declaration changes
	   the declaration, generate the stabs info (code stolen from Vax) */
	changed = 0;
...
# endif
	...
	uerror( "redeclaration of %.32s", p->sname );
...
# endif
	if( type == UNDEF ) uerror("void type for %.32s",p->sname);
	p->stype = type;
	p->sclass = class;
	p->slevel = blevel;
	p->offset = NOOFFSET;
...
/** SGI (GB) - if we want to reserve a register variable for the 
	SKY board, this is where we do it..... 
**/

	minrvar = regvar = MAXRVAR | ((reservea5)? ((MAXRVAR-3)<<8):
			((MAXRVAR-2)<<8));
	reached = 1;
	swx = 0;
	swp = swtab;
...
# ifndef BUG1
	if( ddebug ){
		printf( "dclstruct( ");
		printf( NAME_FMT, (i>=0)? stab[i].sname : "??"); 
		printf( " ), szindex = %d\n", szindex );
		}
# endif
	...
	for( i = oparam+4;  i< paramno; ++i ){
		...
		if( sz == 0 ){
			uerror( "illegal zero sized structure member: %.32s", p->sname );
			}
...
# ifndef BUG1
	if( ddebug>1 ){
		...
		for( i = dimtab[szindex+1]; dimtab[i] >= 0; ++i ){
			printf( "\tmember ");
			printf( NAME_FMT, stab[dimtab[i]].sname);
			tprint(stab[dimtab[i]].stype);
			printf(" (%d)\n", dimtab[i] );
			}
		}
# endif
...
ftnarg( idn ) {
	...
	case UNDEF:
		...
		break;
	case FARG:
		uerror("redeclaration of formal parameter, %.32s",
			stab[idn].sname);
		/* fall thru */
	case FTN:
...
/* GB - BUG47. set iclass to ensure we dont get a syntax error and cause
   an init to be done when we haven't beginit'ed */
int iclass = (-1);  /* storage class of thing being initialized */
...
# endif
	...
	case AUTO:
	case REGISTER:
	/* set iclass to illegal init in case the next pass has a syntax
		error and doesn't initialize the pstk. GB BUG47. 4/30/84 */
		iclass = (-1);
		return;
		}
...
/****** BUGREPORT #12 - Structures are not aligned to 32 bits when
	passed as a parameter.  If the structure is < 32 bits, the
	alignment should not be done. GB (SGI) 7/26/83.
		if( (p->sclass==PARAM || p->sclass==REGISTER) && ( tsz < SZINT ) ){
*****************************/
		if( (p->sclass==PARAM || p->sclass==REGISTER) 
		  && ( tsz < SZINT ) && (p->stype != STRTY) ){
...
falloc( p, w, new, pty )  register struct symtab *p; NODE *pty; {
	...
	if ((new >= 0)&&(!(ISUNSIGNED(type)))&&(!isenum)) {
		werror("field '%s' - bitfields are inherently unsigned.",
			p->sname);
...
#if 1
	/* support for scr2072 */
	unsg = INT;  /* INT, UNSIGNED, or SIGNED */
#else
...
/* GB - enhancement for Clover. Ignore (effectively) the keyword 'signed'.
   see also manifest.h cgram.y and scan.c
*/
		case SIGNED:
#if 1
			/* support for scr2072 */
...
/* GB bug #75 - give better diagnostics.  changed interface
   to fixclass to include node.  Only call is in this module. */
fixclass( class, q )  register NODE *q; { /* does q's class need changing? */

	TWORD type = q->in.type;
	...
	case REGISTER:
		/* GB (bug #75) - give warning if cant alloc register variable */
		if( blevel == 0 ) uerror( "illegal register declaration" );
		else if ( ISPTR(type) ) {
		  if( ((regvar>>8)&0377) >= MINARVAR && cisreg( type ) ) return( class );
		  else  {
			cwerror(
				"too many pointer register variables in function %s. Max 4.",
				getftnnm());
			cwerror(
			"register declaration for variable %.32s in function %s ignored.",
				stab[q->tn.rval].sname,getftnnm());
		  }
		} else {
		  if( (regvar&0377) >= MINDRVAR && cisreg( type ) ) return( class );
		  else {
			cwerror(
				"too many register variables in function %s. Max 6.",
				getftnnm());
			cwerror(
			"register declaration for variable %.32s in function %s ignored.",
				stab[q->tn.rval].sname,getftnnm());
...
# endif
	...
	/* the wordsize... */
/*####################
	the above comment (sigh) is not true, since we now have
	32 chars of significance.
#####################*/
	if (i<0) i=(-i);		/* gb (SGI) 4/8/83 */
...
checkst(lev){
	...
	for( i=0, p=stab; i<SYMTSZ; ++i, ++p ){
		...
		if( j != i ){
			...
			if( q->stype == UNDEF ||
			    q->slevel <= p->slevel ){
				cerror( "check error: %.32s", q->sname );
				}
			}
		else if( p->slevel > lev ) cerror( "%.32s check at level %d", p->sname, lev );
		}
	}
#endif
	...
	for(;;){
		...
		if( p->slevel>lev ){ /* must clobber */
			if( p->stype == UNDEF || ( p->sclass == ULABEL && lev < 2 ) ){
				lineno = temp;
				uerror( "%.32s undefined", p->sname );
				}
			else aocode(p);
# ifndef BUG1
			if (ddebug) printf("removing %32s from stab[ %d], flags %o level %d\n",
...
hide( p ) register struct symtab *p; {
	...
	if( hflag ) werror( "%.32s redefinition hides earlier one", p->sname );
# ifndef BUG1
