...
p2init( argc, argv ) char *argv[];{
	...
	for( c=1; c<argc; ++c ){
		...
		debugging =  (edebug || odebug || radebug || rdebug ||
			tdebug || sdebug || udebug || xdebug) ;
...
/***** GB SGI - UNI added debug info */
#ifndef BUG4
		if (odebug)
			printf("order: U* deleted\n");
#endif
...
# ifndef BUG4
			if (odebug)
				printf("order: rewrite INCR or DECR\n");
# endif
...
# ifndef BUG4
			if (odebug)
				printf("order: INCR or DECR for effect\n");
# endif
...
/****** GB SGI (#44) need to get rid of the assignment in the right
	subop   *****/
		p->in.right->in.op = NOASG(p->in.right->in.op);
/******/
		canon(p);
		rallo( p, p->in.rall );
...
/*****	GB SGI (#55) We have a problem here.  The right subtree
	may very well be a CALL. In this case, we cant have the
	left subtree evaluated first into a reg, because all
	registers must be dead at a CALL.  Forcing the left
	subtree evaluation at this point into a temp register 
	guarantees that it is an rval when the operator node is
	evaluated.  We could evaluate it into a temp location, but
	it will usually have to be moved to a register later (after
	the call.)  This isnt the best solution (in the code generated),
	as the left may be simple, but anyone mixing up types like this
	deserves what they get.
		order( p2->in.left, INTBREG|INTAREG );
******/
		if( p->in.type == INT )
			order( p2->in.left, INTBREG|INTAREG );
		else switch( p->in.right->in.op) {
			case RS: case LS:
			case PLUS: case MINUS:
				order( p2->in.left, INTBREG|INTAREG );
				break;
			default:
				order( p2->in.left, INTEMP );
				break;
				}
...
#ifdef SGI_REGS
	/* if cook is FORARG and we have just generated an arg in 
	   a register, the tree is finished.  If the register
	   is the desired one, free the tree. */

/* *** GB REG ***/
	if ( (cook & FORARG) && 
		 ((!(p->in.node_sgi & HWOP)) || (fpatype == NO_FPA)) &&
	       ((p->in.node_sgi & (SGIARG)) )) 
	    if ((p->in.op == REG) && (p->tn.rval == (p->in.node_sgi & 0xFF))){
			/* this register is really still busy.  We get away with
		   	   it for other FORARGS because Z- in zzzcode does 
			   not reserve the register */
#ifdef NOTDEF
			rbusy(p->tn.rval,INT);
			if (p->in.type == DOUBLE)
				rbusy(p->tn.rval + 1,INT);
#endif
			if (((ty = p->in.type)==CHAR)||(ty==SHORT) ||
		   			(ty==UCHAR)||(ty==USHORT)) {
				register NODE *q;
				NODE *block();
	...
	case GT:
		...
		if( p->in.right->in.op == ICON && 
			...
			if( p->in.left->in.op == ICON && 
				...
				if (o > ULE) {
					rescon =
					  (unsigned long)p->in.left->tn.lval - 
					  (unsigned long)p->in.right->tn.lval;
					o -= (ULE - LE);
				}
				else {
					rescon =
...
#ifdef NOTDEF
			else if (p->in.right->tn.lval == 0) {
				/* comparison of zero to non-constant */
...
eprint( p, down, a, b ) NODE *p; int *a, *b; {
	...
	*a = *b = down+1;
	...
	case FLD:
		printf("field: off = 0x%x, sz = 0x%x ",
			UPKFOFF(p->tn.rval),UPKFSZ(p->tn.rval));
		break;
...
#ifdef SGI_REGS
	if ((rall = p->in.node_sgi)  ) 
		printf("SPECIAL_SGI %x (hwop = %x) ",rall,p->in.hw_opcode );
#endif
...
#define FIELDOPS 0 	/* GB - let fields survive....dont remove them */
#ifndef FIELDOPS
