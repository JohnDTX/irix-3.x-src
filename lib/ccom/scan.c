...
/***** 	GB (#39) UNI  put asms in grammar. it is now a reserved word.
# define AR_A 6		 asm 
*****/
...
# ifdef ONEPASS
	p2init( argc, argv );
# endif
	debugging = (debugging || ddebug || idebug || bdebug ||
			tdebug || edebug || xdebug);

	if (debugging) errfile = stdout;
...
#ifdef NOTDEF
	/* GB - ejobcode now checks errno and returns it 
	ejobcode( nerrors ? 1 : 0 );
	*/
	nerrors = ejobcode( nerrors ? 1 : 0 );
#else
	ejobcode (nerrors ? 1: 0);
#endif
	return(nerrors?1:0);
...
/***** 	GB SGI asm is now a reserved word (#39) UNI ****
	strflg is <0 if the char is part of an asm.
*****/
#ifndef LINT
				if (strflg<0) 
					putchar(val) ;
	...
	if( lxmatch == '"' ){
/***** 	GB SGI asm is now a reserved word (#39) UNI ****
	strflg is <0 is the char is part of an asm.
*****/
#ifndef LINT
		if (strflg<0) putchar('\n');

		else if( strflg ){ /* end the string */
...
/* GB - enhancement for Clover. Ignore (effectively) the keyword 'signed'.
   see also pftn.c cgram.y and manifest.h (9/15/86)
*/
	"signed",	AR_TY,	SIGNED,
	"sizeof",	AR_RW,	SIZEOF,
	"short",	AR_TY,	SHORT,
	...
	for( p= lxrdope+c; p->lxrch[0] == ch; ++p ){
		if( !strcmp( yytext, p->lxrch ) ){ /* match */
			...
			case AR_TY:
				...
				{	
			  		cwerror(
	"use 'long float' for 64-bit float quantities: float assumed");
					yylval.nodep = mkty( (TWORD) FLOAT,
							    0, FLOAT);
				}
				else
#endif
...
/******	GB SGI (#39) asm is now a reserved word.  deleted AR_A section.
	see .v6 copy for deleted section
 ****/
...
lxtitle(){
	/* NEW lxtitle for stabs.  Sets source name when filename 
		first occurs, then ignores the linenumbers on any cpp
		line whose title does not match the first one encountered. */
	/* called after a newline; set linenumber and file name */
