...
private inittypes()
{
    ...
    maketype("long float", builtintype(DOUBLE), t_int, 8L, 0L);
#endif
    t = builtintype(UNDEF);
#ifdef NEEDDBXINIT
...
register struct symtab *p;
{
	...
	switch (p->sclass) {
	    ...
	    case EXTDEF:
		if (ISFTN(p->stype)) {
/* GB BUG99. functions declared with the class 'fortran' have no leading
...
int szindex, paramindex;
{
	...
	i = dimtab[szindex + 3];
	p = &stab[i];
/*** GB - bug in ccom for how long?  if i<0, struct is unnamed 
	if (p->sname != nil) {
***/
	if ((i >= 0)&&(p->sname != nil)) {
	    strindex = dimtab[p->sizoff + 1];
