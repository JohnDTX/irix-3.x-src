...
#define NAMESIZE 32	/* namesize is a local mod to make symbols > 8 chars */
...
char buffer[NAMESIZE+BUFSIZ+BUFSIZ+NAMESIZE];
...
/* DR lastchar - last char dumped - checked if equal to NL
 *               when speakup used to 
 *               call sayline.  Fixes long macro bug, line number
 *               printed in middle of line 
 */
char lastchar;
...
/* GB (7/24/86) - increase the max number of -I dirs to 16, and parameterize it.. */
#define MAXIDIRS 0x10
#define MAXSTDDIRS 0x3		/* ".", /usr/include, /usr/local/include. */
STATIC	char	*dirs[MAXIDIRS+MAXSTDDIRS+1];	/* -I and <> directories */
char *strdex(), *copy(), *subst(), *trmdir();
struct symtab *stsym();
STATIC	int	fin	= STDIN;
STATIC	FILE	*fout	= stdout;
STATIC	int	nd	= 1;
STATIC  int	nodefaults = 0;	/* inhibit -I/usr/include */
...
#ifdef PASCAL
STATIC 	int pascal=0;
STATIC 	int pcommtype = 0;
#define BRACE 1
#define PAREN 2
STATIC  int dont_echo_lines=0;
#endif PASCAL
		...
		if (mactop>inctop[ifno]) {/* retrieve hunk of pushed-back macro text */
			...
			if (ifno==0) {/* end of input */
				if (plvl!=0) {
					...
					*np++ = '*'; *np++ = '/';	/* shut off unterminated comment */
...
cotoken(p) register char *p; {
	/**
		cotoken - this routine appears to return only when
		the next control token is found.
...
again:
...
#ifdef PASCAL
				if ((pascal)&&(ifno)) {
					/* see if this close comment is the end-of-line */
					/* this will work UNLESS we hit the end-of-block
					   between here and eol, and nothing significant
					   intervenes */
					while ((toktyp+COFF)[*p++] & BLANK);
					p--;
#ifdef DEBUG
					fprintf(stderr,"checking end of C comment in pascal\n");
					fprintf(stderr,"first signif char was 0x%x\n",*p);
#endif
					passcom = lvcmsv;
					if (*p == '\n') {
						inp = p;
	...
	if (!pascal) break;
#ifdef DEBUG
	fprintf(stderr,"pascal comment begun\n");
#endif
		for (;;) {
		if ((*inp == '{')||(*p++=='*')) {/* comment */
#ifdef PASCAL
	pcommtype = (*inp == '{'?BRACE:PAREN);
#ifdef DEBUG
	fprintf(stderr,"pascal comment seen, type = '%c'\n",(*inp == '{')?'{':'(');
#endif
#endif
			if (!passcom) {inp=p-2; dump(); ++flslvl;}
			for (;;) {
				while (!iscom(*p++));
				if ((pcommtype == BRACE)&&(p[-1]=='}')) goto pendcom;
				if ((pcommtype == PAREN)&&(p[-1]=='*')) for (;;) {
					if (*p++==')') goto pendcom;
					if (eob(--p)) {
		...
		pendcom:
#ifdef DEBUG
			fprintf(stderr,"end of pascal comment seen\n");
#endif
...
prevlf:
		if (speakup) {
			inp=p; dump(); lineno[ifno]=speakup+1;
			if (lastchar != '\n') putc('\n',fout);
			sayline(CONT); speakup=0;
		}
		state=BEG;
		for (;;) {
			if (*p++=='#') return(p);
...
#ifdef DEBUG
	fprintf(stderr,"in doincl, begin with line# = %d, char 0x%x\n",
				lineno[ifno],*p);
#endif
...
# endif
	...
	++flslvl; 
#ifdef DEBUG
	fprintf(stderr,"at end of include file, char is 0x%x, 0x%x\n",*inp,*p);
#endif
#ifdef PASCAL
	if ((pascal)&&(ifno)) {
		while (*inp != '\n') {outp=inp=p; p=cotoken(p);} 
#ifdef DEBUG
		fprintf(stderr,"doincl: trying to flush \\n: 0x%x\n",*p);
#endif
		p++;
#ifdef DEBUG
		fprintf(stderr,"new char is %c (0x%x)\n",*p,*p);
#endif
		outp=inp=p;
	}
	else
#endif
	{
		do {outp=inp=p; p=cotoken(p);} while (*inp!='\n'); 
		inp=p; 
	}
	--flslvl;
	dump(); 
...
dodef(p) char *p; {/* process '#define' */
	...
	if (savch>sbf+SBSIZE-BUFSIZ) {pperror("too much defining"); 
	return(p);}
...
for (;;) {
	...
	*--inp=SALT; outp=inp; ++flslvl; np=slookup(inp,p,0); --flslvl;
	if (np==defloc) {/* define */
		...
		/* GB (SGI) SCR1133.  yyparse() has to have
		   comment eliding ON always.  Save and restore
		   the 'pass comment' flag around the call to 
		   yyparse()
		*/
		int passcomsv = passcom;	/* SCR1133 */
		passcom = 0;			/* SCR1133 */
#if tgp
	...
	else if (flslvl==0)	/* if we're not expanding, anything goes */
...
slookup(p1,p2,enterf) register char *p1,*p2; int enterf;{
	...
	if ((p2-p1)>NAMESIZE) p3=p1+NAMESIZE; else p3=p2;
			 c3= *p3; *p3='\0';	/* truncate to NAMESIZE chars or less */
...
subst(p,sp) register char *p; struct symtab *sp; {
	...
	if ((p-macforw)<=macdam) {
		if (++maclvl>maxrecur && !rflag) {
			if (!usermaxrecur) pperror("%s: macro recursion",sp->name); 
			return(p);
		}
...
#if scw2
			/* 54 == 64-10; digits rarely appear in identifiers,
			/* and can never be the first char of an identifier.
			/* 12 == 54*54/sizeof(macbit) .
			*/
			++i; (t21+COFF)[c]=(54*i)/12; (t22+COFF)[c]=i%12;
#endif
		}
	p="0123456789.";
...
# else
			switch(argv[i][1]) {
# endif
				case 'M': mflag++;	/* FALL THROUGH */
				case 'P': pflag++;
						  continue;
				case 'N': /* GB (sgi) -N indicates
							 'dont give -I/usr/include '
						  */
						  nodefaults++;
				case 'E': continue;
				case 'R': 
					if (argv[i][2]) {
						/* GB */
						/* -R<num> defines max number of recursions */
						maxrecur = atoi(&argv[i][2]);
						usermaxrecur++;
					} else
						++rflag; 
					continue;
...
#ifdef PASCAL
				case 'p':
					/* pascal source - recognize pascal comments */
#ifdef DEBUG
					fprintf(stderr,"pascal turned on\n");
#endif
					pascal++;
					/* dont output line number directives,
					   either */
					pflag++;
					continue;
#endif PASCAL
...
#ifdef PASCAL
	if (pascal) (fastab+COFF)['}'] |= CB|SB; 
	p = "({"; while (c= *p++) (fastab+COFF)[c] |= SB;
#endif PASCAL
...
# if unix
	/* GB (sgi) */
	if (!nodefaults) dirs[nd++] = "/usr/include";
# ifdef LOCAL
...
# endif
	...
	fnames[ifno]=tf;
	pbeg=buffer+NAMESIZE; pbuf=pbeg+BUFSIZ; pend=pbuf+BUFSIZ;
