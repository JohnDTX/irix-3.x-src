...
register char *p;
{
	 ...
	 * be represented, and then 2^(-55) jammed via _lldexp().
	 */
	flexp = 1;
	exp5 = 5;
	bexp = exp;
	...
	if(negexp < 0)
		...
		fl *= flexp;
	fl = _lldexp(fl, negexp*bexp);
	if(neg < 0)
		fl = -fl;
	return(fl);
}

float atof(p) register char *p;
{
	long float lretval;
	if ((lretval = _latof(p)) > (long float)MAXFLOAT) 
		return(_raise_fperror(CONVERT,OVERFL));
	else return((float)lretval);
}
