...
#include <nan.h>
#include <values.h>
#define	NMAX	((FSIGNIF * 3 + 19)/10) /* restrict max precision */
#define	NDIG	80
...
register int	*decpt;
{
	...
	*decpt = 0;
	if (value != 0.0) { /* rescale to range [1.0, 10.0) */
		...
		static struct s { float p10; int n; } s[] = {
			1e32,	32,
			1e16,	16,
			1e8,	8,
			1e4,	4,
		...
		++*decpt;
		if (value >= 2.0 * FMAXPOWTWO) /* can't be precisely integral */
			do {
				for ( ; value >= sp->p10; *decpt += sp->n)
#ifdef SW_DIV
					_sw_f_idiv(&value,sp->p10);
#else
					value /= sp->p10;
#endif
			} while (sp++->n > 1);
			...
			register 
#endif
			float pow10 = 10.0, powtemp;

			while ((powtemp = 10.0 * pow10) <= value)
				pow10 = powtemp;
#ifdef SW_DIV
			for ( ; ; _sw_f_idiv(&pow10,(float)10.0)) {
				register int digit = (int)_sw_f_div(value,pow10);
#else
			for ( ; ; pow10 /= 10.0) {
