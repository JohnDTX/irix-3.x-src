...
/*
|
|	fadd.s 	- H/W floating point add routines for the Juniper
|		  FPA.  The following entries are in this module:
|
|		_f_add	-  add two floats passed on the stack.
|
|	    register routines:
|		_fr_add -  add two floats passed in d0/d1.
|
|	    indirect routines:
|		_fr_iadd - add the float in d0 to that whose address
|			   is in a0.
|
|
|
...
|	uses f1 
|
	fwritefpa(sp@(4),1)	| move first addend to f1
	faddx1upd(1,sp@(8))	| add in second addend and update f1 with result
	freadfpa(1,d0)
	rts
|
|
...
|	d1	- second addend
|	
	fwritefpa(d1,1)		| move first addend to f1
	faddx1upd(1,d0)		| add in second addend and update f1 with result
...
|
	...
	fwritefpa(sp@(8),1)	| move first addend to f1
	faddx1upd(1,a0@)	| add in indirect second and update f1
...
|
	fwritefpa(d0,1)		| move first addend to f1
	faddx1upd(1,a0@)	| add in indirect second and update f1
	freadfpa(1,a0@)		| store result indirect
	movl	a0@,d0
	rts
