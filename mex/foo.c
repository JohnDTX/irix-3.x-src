...
#define SIGKID	SIGUSR1
	...
	sleep(5);		/* give child more than enough time to setup */
	printf("wokeup\n");
	exit(0);
    }
...
gettiedstuff()
{
	...
	softqenter(dev,val);
	fprintf(stderr,"mex: X valuator not here.[%d]\n",dev);
    }
    dev=qread(&val);
    if(dev == CURSORY) 
	mousey = val;
	...
	softqenter(dev,val);
	fprintf(stderr,"mex: Y valuator not here.[%d]\n",dev);
    }
