...
** $Author: root $
** $State: Exp $
** $Source: /d2/3.7/src/pm2stand/mdfex/RCS/sets.c,v $
** $Revision: 1.1 $
** $Date: 89/03/27 17:11:48 $
*/
...
setv()
{
	...
	for(;;) {
		printf(" Set ? ");
		switch(gch()) {
		case 'a':	gioport(); break;
		case 'b':	spbads(); break;
		case 'd':	defaults(); break;
		case 'f':	fmware(); break;
		case 'h':	shelp(); break;
		case 'i':	silv(); break;
		case 'l':	slabel(); break;
...
#ifdef NOTDEF
printf("  addr       - i/o port (and WUB) address\n");
printf("  badblocks  - display current drive bad block list\n");
printf("  firmware   - disable firmware retries\n");
printf("  interleave - set interleave\n");
#endif NOTDEF

printf("  defaults   - display settings\n");
printf("  help       - this message\n");
printf("  mode       - mode of disk address entry\n");
printf("  quit       - quit and return to main routine\n");
printf("  randreset  - reset random number sequence\n");
printf("  type       - select type of drive (? for list)\n");
printf("  unit       - select unit for testing (0/1/f)\n");
printf("  verbose    - verbose (on/off)\n");
printf("  writelock  - write lock switch (on/off)\n");
...
#ifdef NOTDEF
char *DTypes[] = {
	"Atasi 3046","Vertex V170", "(not used)", "(not used)",
...
slabel()
{
	...
	l->d_rootfs = grootswap("Root", (l->d_rootnotboot)? l->d_rootfs:
					l->d_bootfs);
	l->d_rootnotboot = 1;
