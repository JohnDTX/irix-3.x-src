/*
** $Source: /d2/3.7/src/stand/cmd/ipfex/RCS/sets.c,v $
** $Date: 89/03/27 17:12:29 $
** $Revision: 1.1 $
*/
...
setv()
{
	...
	printf(" Set ? ");
		...
		case 'f':	if (password) fquiet(); break;
		case 'g':	if (password) sgaps(); break;
		case 'i':	if (password) silv(); break;
		case 'j':	if (password) slabel(); break;
		...
		case 'D':	if (password) sdirect(); break;
		case 'G':	if (password) sgroup(); break;
		case '#':	if (password) shelp(); break;
		case 'h':
		case '?':
		default:	shelp(); break;
		}
...
printf("\n");
printf("    *** Set Commands ***\n");
printf("    l - set disk file systems sizes\n");
printf("    q - quit - return\n");
printf("    t - select type of disk\n");
printf("    u - select disk unit for testing\n");
printf("\n");

if (!password) return;
printf("    a - display settings\n");
printf("    b - bad block list\n");
printf("    c - set cache on or off and also groupsize\n");
printf("    f - quiet all extraneous printout (on/off)\n");
printf("    g - gap settings\n");
printf("    i - interleave settings\n");
printf("    j - set complete disk label\n");
printf("    m - mode of disk address entry\n");
printf("    r - reset random number sequence\n");
printf("    s - set spiral skew factor\n");
printf("    v - verbose (on/off)\n");
printf("    w - write lock switch (on/off)\n");
printf("    D - Turn on/off direct mode\n");
printf("    G - Set the groupsize\n");
}
