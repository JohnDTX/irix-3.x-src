/*
 * $Source: /d2/3.7/src/stand/lib/gl/RCS/gl2gfdev.h,v $
 * $Revision: 1.1 $
 * $Date: 89/03/27 17:15:10 $
 */
#ifndef GF2DEF
#define GF2DEF
/*--------------------------------------------------------------------------*/
...
#ifdef IP2
#define GFMBM(x)	( SEG_MBIO + (x) )
#define GEPORT		*(unsigned short *)0x6001000	/* write-only */
#define GETOKEN		*(unsigned short *)0x6000000	/* write-only */
#endif

#ifdef PM2
#   ifdef V /* v kernel on pm2 */
#       define GFMBM(x)		(0x30000 + (x))
#       define gewait()		asm(".word 0x3039, 0x0003, 0x2C00, 0x6BF8")
#   endif V
#   ifdef UNIX
#   	/* unix stuff on pm2 */
#       ifdef KERNEL
#           define GFMBM(x)	(0xEEE000 + (x))
#           define gewait()	asm(".word 0x3039, 0x00EF, 0x0C00, 0x6BF8")
#       else
...
#       define gewait()	asm(".word 0x3039, 0x00F7, 0x2C00, 0x6BF8")
#   endif PROMALONE
#   ifdef GLUSER
#       define GFMBM(x)	(0xEF4000 + (x))
#       define gewait()	asm(".word 0x3039, 0x00EF, 0x6C00, 0x6BF8")
#   endif GLUSER
...
#    ifdef KERNEL
#        define gewait()	asm(".word 0x3039, 0x001F, 0x2C00, 0x6BF8")
#    endif
#    define GEPORT	*(unsigned short *)0x401000	/* write-only */
#    define GETOKEN	*(unsigned short *)0x400000	/* write-only */
#endif
