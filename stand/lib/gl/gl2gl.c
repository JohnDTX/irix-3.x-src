/*
 * $Source: /d2/3.7/src/stand/lib/gl/RCS/gl2gl.c,v $
 * $Revision: 1.1 $
 * $Date: 89/03/27 17:15:11 $
 */
# undef  DIMTEST
/*
 * gl2gl.c --
...
/*
 * the DC4 has a concept of a primary and secondary monitor.  Below are
 * the bits that need to be stuffed into bits 11 to 14 of the dcr register.
 * Because there are two types of monitors the bits are stuffed into one
 * byte.  This code was probably first written by a hardware type that wanted
 * to save every nibble.  It should be made clearer
 *
 * Things are not as simple as one would like.  You need to stuff different
 * bits into the dcr based on the combination of primary and secondary
 * displays.  This array assumes that NI is 0, I - 1, NTSC - 2, PAL - 3;
 * The primary monitor must be a 30 or 60 Hz monitor.  The secondary monitor
 * can be any of the 4.
 *
 * The procedure here is to determine the P/S display combination and extract
 * the timing bits from the following table.  Determine what display  we are
 * using (ie P or S) and then extract the bits from the correct nibble and
 * shift them over 11 bits so you can setup the DC4 register. The packed byte
 * is passed to the kgl so that it can switch between monitor types. The bits
 * in the upper nibble are for the secondary display.  The bits in the lower
 * nibble are for the primary display.
 * I wonder when people will learn that memory is cheap and that you don't
 * have to play bit stuffing games.
 */

#define PACKBITS(primary,secondary) \
	( ((((secondary) >> 11) & 0x0f) << 4) | ((primary) >> 11) & 0x0f )

/* NOTE: The order of this array is linked to the switch settings see 
 * cpureg.h
 */
u_short dcr_bits[] = {
	PACKBITS( (PROM | PIPE4), (PROM | PIPE4) ),	/* NI/NI */
	PACKBITS( (PROM | PIPE4), 0),			/* NI/I */
	PACKBITS( PIPE4, (PROM | OPTCLK) ),		/* NI/NTSC */
	PACKBITS( PIPE4, (PROM | OPTCLK) ),		/* NI/PAL */
	PACKBITS( 0, (PROM | PIPE4) ),			/* I/NI	*/
	PACKBITS( 0, 0),				/* I/I	*/
	PACKBITS( 0, (PROM | OPTCLK) ),			/* I/NTSC */
	PACKBITS( 0, (PROM | OPTCLK) ),			/* I/PAL */
};
...
#endif
	/* set up the stuffed dcconfig bits that are passed to the kgl
	 * this code is really stupid.  You should just configure the dc4
	 * and then pass unix the primary and secondary display types.
	 * However, we need to do this bit stuffing to maintain compatabilty
	 */
...
/* return the packed dc4 timing bits for the display pair combination */
u_char
getdcrbits(discombo)
{
	return(dcr_bits[discombo]);
}
