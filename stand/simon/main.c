/*
* $Source: /d2/3.7/src/stand/simon/RCS/main.c,v $
* $Revision: 1.1 $
* $Date: 89/03/27 17:20:54 $
*/
#include	"sys/types.h"
#include	"cpureg.h"
#include	"common.h"
...
skipit:
	...
	*(u_short *)&swregbits = *SWTCH_REG;
	...
	if ( _commdat->c_memmb ) {
		if ( motormouth ) {
			printf( "  Multibus accessable memory (1mb) begins\n" );
			printf( "    at Physical memory page %x,\n"
							,_commdat->c_mbspg);
			printf( "    at Virtual address %x.\n", MBmemVA );

			if ( _commdat->c_memmb == 1 ) {
				printf( "  WARNING: Multibus accessable memory overlaps all\n" );
		...
		*/
		...
		if ( motormouth )
			printf( "  No Multibus accessable memory\n" );
	...
	while ( 1 )
	{
		...
		if ( !strcmp(argv[0], "cat" ) )
		{
		   ...
		   case XNS_CMD:			/* PM2 compatability */
			/* all of this groddy code is to fake up argv */
