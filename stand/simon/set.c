/*
* $Source: /d2/3.7/src/stand/simon/RCS/set.c,v $
* $Revision: 1.1 $
* $Date: 89/03/27 17:20:58 $
*/
...
todraminit()
{
	...
	if ( !(tod_dev.td_regD & RD_VRT) || magic != SI_MAGIC ) {
		...
		sip->tds_magic[0] = (SI_MAGIC >> 24) & 0x0ff;
		sip->tds_magic[1] = (SI_MAGIC >> 16) & 0x0ff;
		sip->tds_magic[2] = (SI_MAGIC >> 8) & 0x0ff;
		sip->tds_magic[3] = SI_MAGIC & 0x0ff;
		...
		*(u_short *)&swregbits = *SWTCH_REG;
		...
		todwrite(CLK_DATA + ((int)&tod_dev.td_sgiun - (int)&tod_dev),
			(u_char *)&tod_dev.td_sgiun, sizeof(struct td_sginfo) );
	} else {
