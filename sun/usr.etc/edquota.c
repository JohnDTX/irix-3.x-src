#ifndef lint
/* @(#)edquota.c	2.1 86/04/17 NFSSRC */
static	char *sccsid = "@(#)edquota.c 1.1 86/02/05 SMI"; /* from UCB 4.5 */
#endif
	...
	register uid;
{
	...
	for (fsqp = fsqlist; fsqp; fsqp = fsqp->fsq_next) {
		if (quotactl(Q_GETQUOTA,fsqp->fsq_dev,uid,&fsqp->fsq_dqb)!=0) {
			...
			if (read(fd, &fsqp->fsq_dqb, sizeof(struct dqblk)) !=
			    sizeof(struct dqblk)) {
				fsqp->fsq_dqb = zdqblk;
			}
			close(fd);
