	...
	
{
...
#ifdef sgi
		strcat(log_filename, "/YPMAPXFR.LOG");
#else
	...
	if (ypdbminit(current_transfer->mx_temp_path) < 0) {
	...
	while (TRUE) {
		...
		if (ypclnt_status) {
			...
			if (ypclnt_status == YPERR_NOMORE) {
				...
				/*
				 * *** SGI NOTE ***
				 * This bug came with SUN NFS 2.0.
				 * No new record is returned on YPERR_NOMORE,
				 * so it is not necessary to do another store.
				 *
				 * NOTE:  the same bug also exists in SUN 3.0.
				 */
				if (store(key, val) < 0) {
				...
				/*
				 * These values are stale, don't free them!
				 */
				free(val.dptr);
				free(key.dptr);
#endif
				...
				if (log_file) {
					fprintf(log_file,
		"failed:  Can't get key-value pair from peer; \n\treason %s.\n",
					    ypclnterr2string(ypclnt_status));
				}
