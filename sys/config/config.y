...
/*
 * $Source: /d2/3.7/src/sys/config/RCS/config.y,v $
 * $Revision: 1.1 $
 * $Date: 89/03/27 17:27:29 $
 */
%union {
	char	*str;
	int	val;
...
Con_info:
	AT Dev NUMBER
	      = {
		if (eq(cur.d_name, "mba") ||
		      eq(cur.d_name, "uba") ||
		      eq(cur.d_name, "mb")) {
			(void) sprintf(errbuf,
				       "%s must be connected to a nexus",
				       cur.d_name);
			yyerror(errbuf);
		}
...
/*
 * yyerror:
 *	- print out error message for line, and then stop
 */
yyerror(s)
	char *s;
{
	...
	register int num;
{
	...
	for (dp = dtab; dp != 0; dp = dp->d_next) {
		...
		if (dp->d_type != CONTROLLER && dp->d_type != MASTER) {
			(void) sprintf(errbuf, "%s connected to non-controller",
					       dev);
			yyerror(errbuf);
			return (0);
		}
	...
	register struct device *dp;
{
	...
	dp->d_slave = dp->d_drive = dp->d_unit =
		dp->d_pri = dp->d_addr = UNKNOWN;
}
...
/*
 * savelibrary:
 *	- given a library name, remember it for later generation in the
 *	  makefile
 */
#define	NLIBRARIES	50
int	libraries_saved;
char	*libnames[NLIBRARIES];
	...
	char *libname;
{
	...
	if (libraries_saved >= NLIBRARIES) {
		fprintf(stderr, "config: too many library specs\n");
		exit(-1);
	}

	cp = (char *)malloc(strlen(libname)+1);
	strcpy(cp, libname);
	libnames[libraries_saved++] = cp;
}

do_libraries(ofp)
	FILE *ofp;
{
	register int i, len, clen;

	fprintf(ofp, "LIBS= ");
	len = 6;
	for (i = 0; i < libraries_saved; i++) {
		clen = strlen(libnames[i]);
		if (len + clen > 75) {
			fprintf(ofp, "\\\n\t%s ", libnames[i]);
			len = clen + 8;
		} else {
			fprintf(ofp, "%s ", libnames[i]);
			len += clen + 1;
		}
	}
	fprintf(ofp, "\n\n");
...
{
	if (graphics_name) {
		fprintf(stderr, "config: already specified graphics\n");
		exit(-1);
	}
	if ((strcmp(glname, "gl1") == 0) ||
	    (strcmp(glname, "gl2") == 0)) {
		graphics_name = (char *)malloc(strlen(glname)+1);
		strcpy(graphics_name, glname);
	} else {
		fprintf(stderr,
			"config: graphics type not \"gl1\" or \"gl2\"\n");
		exit(-1);
	}
}
