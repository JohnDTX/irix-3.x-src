/*
 * $Source: /d2/3.7/src/sys/config/RCS/mkioconf.c,v $
 * $Revision: 1.1 $
 * $Date: 89/03/27 17:27:32 $
 */
#ifndef lint
...
#if defined(MACHINE_PM2) || defined(MACHINE_IP2)
sgi_ioconf(machine)
	int machine;
{
	register struct device *dp, *mp;
	...
	if (machine == MACHINE_PM2 || machine == MACHINE_IP2) {
		fprintf(fp, "#include \"../h/param.h\"\n");
		fprintf(fp, "#include \"../h/buf.h\"\n");
		fprintf(fp, "#include \"../h/map.h\"\n");
		fprintf(fp, "#include \"../multibus/mbvar.h\"\n");
		fprintf(fp, "\n");
	}
	/*
	 * Now generate interrupt vectors for the Multibus
...
"\t/* driver,  unit, ctlr,  slave,   addr,    pri,    dk, flags*/\n");
	for (dp = dtab; dp != 0; dp = dp->d_next) {
		...
		fprintf(fp,
"\t{ &%sdriver,  %2d,   %s,    %2d,   0x%x, %d,  %d,  0x%x },\n",
		    eq(mp->d_name, "mb") ? dp->d_name : mp->d_name, dp->d_unit,
		    eq(mp->d_name, "mb") ? " -1" : qu(mp->d_unit),
		    slave,
		    (dp->d_addr == UNKNOWN) ? 0 : dp->d_addr,
		    (dp->d_pri == UNKNOWN) ? 0 : dp->d_pri,
		    dp->d_dk, dp->d_flags);
	}
	fprintf(fp, "\t0\n};\n");
	(void) fclose(fp);
