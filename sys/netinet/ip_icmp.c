...
{
#ifdef sgi
	extern int time_ticks;		/* ticks since start of second */
	extern long time_usec;		/* usec since start of tick */
	u_long t;
	int s = spl7();			/* get consistent ticks */
	t = (time % (24*60*60))*1000 
		+ (time_ticks*(1000000/HZ) + time_usec)/1000;
	splx(s);
#else
	struct timeval atv;
	u_long t;
