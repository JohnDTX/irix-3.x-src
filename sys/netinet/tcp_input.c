...
#define	ourfinisacked	(acked > 0)
		...
		if (tp->t_rtt && SEQ_GT(ti->ti_ack, tp->t_rtseq)) {
			if (tp->t_srtt == 0)
				...
				tp->t_srtt =
#ifdef sgi
				    (tcp_alpha * tp->t_srtt + SRTT_SCALE/2)
				    / SRTT_SCALE
				    + (1*SRTT_SCALE - tcp_alpha) * tp->t_rtt;
#else
				    tcp_alpha * tp->t_srtt +
...
#ifdef sgi
			register int rtt = ((tcp_beta * tp->t_srtt 
					     + SRTT_SCALE*SRTT_SCALE/2)
					    / (SRTT_SCALE*SRTT_SCALE));
