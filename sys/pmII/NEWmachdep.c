/*
 * $Source: /d2/3.7/src/sys/pmII/RCS/NEWmachdep.c,v $
 * $Revision: 1.1 $
 * $Date: 89/03/27 17:33:27 $
 */
#include "../h/param.h"
#include "../h/systm.h"
#include "../h/dir.h"
...
extern	char *chunk_bitmap;
extern	struct pte *chunk_pt;
extern	int chunk_size;
extern	int chunk_maxchunks;
...
#endif
	/*
	 * Now allocate space for chunk management - the compressed
	 * buffer cache.
	 * Calculate the size of the chunk space to manage.  We use 2 times
	 * the so-called maximum size of the cache as the amount to manage.
	 */
	chunk_maxchunks = (nbuf * efs_lbsize * 2 + chunk_size - 1) / chunk_size;
	/* allocate one bit of the bitmap per chunk */
	bytes = (chunk_maxchunks + BITSPERBYTE - 1) / BITSPERBYTE;
	/* allocate one pte for each page of cache memory */
	ptes = btoc(nbuf * efs_lbsize * 2);
	valloc(chunk_bitmap, char, bytes);
	valloc(chunk_pt, struct pte, ptes);
...
caddr_t p;
{
	...
	n = regp[SP] - 6;
	sr = (short)regp[RPS];
	if ((subyte((caddr_t)n, sr >> 8) < 0) ||	/* high byte of sr */
	    (subyte((caddr_t)(n+1), sr) < 0) ||		/* low byte of sr */
	    (suword((caddr_t)(n+2), regp[PC]) < 0)) {	/* pc */
#ifdef	ASSERT
		printf("sendsig failed during push of signal info\n");
		debug("sendsig");
#endif
		;
	}
	...
	short sr;
{
	...
	coswitch(SECCONSOLE);	/* switch to the secondary console */
	...
	int (*strat)(), prio;
{
	...
	if ((bp->b_flags & B_DMA) == 0) {
		...
		bp->b_iobase = mbmapalloc(pte, (long)bp->b_un.b_addr & PGOFSET,
					       bp->b_bcount);
#ifdef	OS_ASSERT
		bp->b_iolen = bp->b_length;
#endif
		bp->b_flags |= B_DMA;
	}
	(*strat)(bp);
