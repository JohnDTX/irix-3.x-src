...
#define	BIGWRITE	512		/* threshold for using clusters */

#ifdef mips
#define pfind procfind			/* XXX is this a kludge? */
#endif
...
#ifdef sgi
			   (uio->uio_resid >= BIGWRITE && space < CLBYTES &&
#else
...
#ifdef sgi
			if (uio->uio_resid >= BIGWRITE && space >= CLBYTES) {
