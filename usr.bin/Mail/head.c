...
char ctypes2[]	= {U,L,L,S,U,L,L,S,O,D,S,D,D,C,D,D,      S,        D,D,D,D,0};

/*		   S u n   A u g     5   2 3 : 4 8 : 5 1   P S T   1 9 8 4 */
char tmztypes[]	= {U,L,L,S,U,L,L,S,O,D,S,D,D,C,D,D,C,D,D,S,A,A,A,S,D,D,D,D,0};

/*		   S u n   A u g     5   2 3 : 4 8         P S T   1 9 8 4 */
char tmztyp2[]  = {U,L,L,S,U,L,L,S,O,D,S,D,D,C,D,D,      S,A,A,A,S,D,D,D,D,0};
	...
	char date[];
{
	...
	if (cmatch(cp, ctypes))
		return(1);
	if (cmatch(cp, ctypes2))
		return(1);
	if (cmatch(cp, tmztypes))
		return(1);
	return(cmatch(cp, tmztyp2));
#else
	if (cmatch(cp, ctypes))
		return(1);
