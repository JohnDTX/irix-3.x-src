...
#define NFILE	10
int	nfile  = 0;			/* index of last used in fl */
FILE	*fl[NFILE]	= {stdin};
...
char **argv;
{
	...
	for (i = 0; i <= nfile; i++)
		if(getnext(i) == 0)
			/* this argv[0] must be a bug, because
			 * it doesn't mean pgm name and it doesn't
			 * really correspond with name of file...
			fprintf(stderr,"%s: read error file %d\n",argv[0],i);
			 */
			if ( i>0 )	/* except for standard in */
