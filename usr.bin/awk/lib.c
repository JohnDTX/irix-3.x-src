...
#define	FINIT	{0, NULL, (awkfloat) 0.0, FLD|STR}
cell fldtab[MAXFLD] = {	/*room for fields */
	{ "$record", record, (awkfloat) 0.0, STR|FLD},
...
char *s;
{
	...
	for (p=s; *p != '='; p++)
		;
	*p++ = 0;
	q = setsymtab(s, tostring(p), (awkfloat) 0.0, STR, symtab);
	setsval(q, p);
...
error(f, s, a1, a2, a3, a4, a5, a6, a7) {
	...
	fprintf(stderr, "\n");
	if (NR && *NR > 0)
		fprintf(stderr, " record number %lg\n", *NR);
	if (f)
		exit(2);
}
