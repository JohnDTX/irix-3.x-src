static	char	*Sccsid = "@(#)$Header: /d2/3.7/src/usr.bin/awk/RCS/run.c,v 1.1 89/03/27 17:43:15 root Exp $";
/*
 * $Log:	run.c,v $
 * Revision 1.1  89/03/27  17:43:15  root
 * Initial check-in for 3.7
 * 
 * Revision 1.2  87/11/01  17:17:36  ciemo
 * Made instat return value of true instead pulling return values out of
 * thin air (or the stack).  This cures a problem with using "continue"
 * which occasionally propagated the "continue" beyond the innermost
 * control loop.
 * 
 * Revision 1.1  85/10/30  16:28:44  brendan
 * Initial revision
 * 
 * Revision 1.3  85/02/05  00:52:17  bob
 * Fixed bug whereby int() function just returned its arg!.
 * 
 */

#include "math.h"
		/* both math.h and awk.def declare some floating routines */
#define	ONCE
#include "awk.def"
#define RECSIZE 512
#include "awk.h"
...
#define MAXTMP 20
cell tmps[MAXTMP];
static cell nullval ={0,0,(awkfloat) 0.0,NUM,0};
...
obj arrayel(a,b) node *a; obj b;
{
	...
	y.optr = setsymtab(s, tostring(""), (awkfloat) 0.0, STR|NUM, x->sval);
	y.otype = OCELL;
	y.osub = CVAR;
	return(y);
}
...
char *format(s,a) char *s; node *a;
{
	...
	int flag = 0;
	awkfloat xf;
	static char fmt2long[] = "format item %.20s... too long";
	...
	while (*s) {
		...
		*t = '\0';
		...
		case 'f': case 'e': case 'g':
			if (s[-1] != 'l') {	/* must have 'l' for long */
				t[-1] = 'l';	/* float (64-bit) sprintf */
				*t++ = *s;
				if (t >= fmt + sizeof(fmt))
					error(FATAL, fmt2long, os);
				*t = '\0';
		...
		if (flag != 4)	/* watch out for converting to numbers! */
			xf = getfval(x.optr);
		if (flag==1)
			sprintf(p, fmt, xf);	/* long float format */
		else if (flag==2)
			sprintf(p, fmt, (long)xf);
		else if (flag==3)
			sprintf(p, fmt, (int)xf);
		else if (flag==4)
			sprintf(p, fmt, x.optr->sval==NULL ? ""
			    : getsval(x.optr));
		tempfree(x);
		p += strlen(p);
		s++;
		...
		for (n = 0; ; ) {
			...
			*s = '\0';
			...
			if (isnumber(t))
				...
				setsymtab(num, tostring(t), (awkfloat) 0.0, STR, ap->sval);
			*s = temp;
			if (*s != 0)
				s++;
		}
...
obj fncn(a,n) node **a;
{
	...
	if (t == FLENGTH)
		...
		u = log(getfval(x.optr));
			...
			/* the "(long)" gets the integer portion */
