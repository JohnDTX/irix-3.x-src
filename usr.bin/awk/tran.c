...
syminit()
{
	setsymtab("0", tostring("0"), (awkfloat) 0.0, NUM|STR|CON|FLD, symtab);
	/* this one is used for if(x)... tests: */
	setsymtab("$zero&null", tostring(""), (awkfloat) 0.0, NUM|STR|CON|FLD, symtab);
	recloc = setsymtab("$record", record, (awkfloat) 0.0, STR|FLD, symtab);
	dprintf("recloc %o lookup %o\n", recloc, lookup("$record", symtab, 0), NULL);
	FS = &setsymtab("FS", tostring(" "), (awkfloat) 0.0, STR|FLD, symtab)->sval;
	RS = &setsymtab("RS", tostring("\n"), (awkfloat) 0.0, STR|FLD, symtab)->sval;
	OFS = &setsymtab("OFS", tostring(" "), (awkfloat) 0.0, STR|FLD, symtab)->sval;
	ORS = &setsymtab("ORS", tostring("\n"), (awkfloat) 0.0, STR|FLD, symtab)->sval;
	OFMT = &setsymtab("OFMT", tostring("%.6lg"), (awkfloat) 0.0, STR|FLD, symtab)->sval;
	FILENAME = &setsymtab("FILENAME", NULL, (awkfloat) 0.0, STR|FLD, symtab)->sval;
	nfloc = setsymtab("NF", NULL, (awkfloat) 0.0, NUM, symtab);
	NF = &nfloc->fval;
	nrloc = setsymtab("NR", NULL, (awkfloat) 0.0, NUM, symtab);
	NR = &nrloc->fval;
}
...
register cell *vp;
{
	...
	if ((vp->tval & NUM) == 0) {
		...
		if (isnumber(vp->sval)) {
			...
			vp->fval = (awkfloat) 0.0;	/* not a very good idea */
	}
	dprintf("  %lg\n", vp->fval, NULL, NULL);
	return(vp->fval);
}
...
register cell *vp;
{
	...
	if ((vp->tval & (NUM | STR)) == 0)
		error(FATAL, "funny variable %o: %s %s %lg %o", vp, vp->nval,
