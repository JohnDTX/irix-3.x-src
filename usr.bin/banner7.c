...
#define MAXMSG 4096
#define DWIDTH 79
#define MAXWIDTH 132		/* must be at least 132 !!! */
#define NCHARS 128
#define NBYTES 9271
...
{ 
	...
	if (argc > 1 && argv[1][0] == '-') {
		...
		case 't':
			...
			break;
		default:
			printf("usage:banner [-wWIDTH] [-d] [-t] message\n");
			exit(-1);
		}
		argc--;
		argv++;
	...
	if (argc > 1) {
		mx = argv[1];
		while (mp < &message[MAXMSG-2] && (*mp++ = *mx++)) ;
		*mp = 0;
		for (i=2; i<argc; i++) {
			*(mp-1) = ' ';
			mx = argv[i];
		...
		while (mp < &message[MAXMSG-2] && (c=getchar())!=EOF&&c!='\n')
			*mp++ = c;
	}
	*mp = 0;
	...
	for (i=0; i<nchars; i++) {
		if (trace)
			printf("Char #%d: %c\n", i, message[i]);
		for (j=0; j<MAXWIDTH; j++) line[j] = ' ';
		_pc = asc_ptr[message[i]];
		term = 0;
		max = 0;
		linen = 0;
		while ( !term ) {
			...
			x = data_table[_pc] & 0377;
			if (trace)
				printf("_pc=%d, term=%d, max=%d, linen=%d, x=%d\n",_pc,term,max,linen,x);
			if (x >= 128) {
				if (x>192) term++;
				x = x & 63;
				while (x--) {
...
#ifdef notdef
				x = (x*width + (MAXWIDTH/2)) / MAXWIDTH;
				y = (y*width + (MAXWIDTH/2)) / MAXWIDTH;
#endif
				max = x+y;
				while (x < max) line[x++] = '#';
				_pc += 2;
