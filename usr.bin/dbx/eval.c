...
static char rcsid[] = "$Header: /d2/3.7/src/usr.bin/dbx/RCS/eval.c,v 1.1 89/03/27 17:44:27 root Exp $";
...
register Node p;
{
    ...
    checkref(p);
    if (traceeval) {
	fprintf(stderr, "begin eval %s sp=%x\n", opname(p->op), sp);
    }
    switch (degree(p->op)) {
	case BINARY:
	    poparg(1, r1, fr1);
...
*/
	...
	case O_SPRINT:
		...
		build_buf = true;
		pbuf_index = 0;
		pbuf_index = sprintf(pbuf, "%s=", 
			Ident(n1->value.arg[0]->nodetype->name));
		printval(n1->value.arg[0]->nodetype);
		build_buf = false;
		shm_write('V', Ident(n1->value.arg[0]->nodetype->name),
			pbuf_index, 0, pbuf);
	    }
	    break;
	...
	case O_PBUFON:
		build_buf = true;
		pbuf_index = 0;
		sprintf(namebuf, "[%d]", current_traceid);
		break;

	case O_PBUFOFF:
		if (pbuf_index > 0) {
			shm_write('V', namebuf, pbuf_index, 0, pbuf);
			pbuf_index = 0;
		}
		build_buf = false;
		break;
	...
	case O_DELETE:
	    n1 = p->value.arg[0];
	    if (n1->op == O_NOP) {
		delevent(0);	/* delete all */
	    } else {
	    	while (n1 != nil && n1->op == O_COMMA) {
			n2 = n1->value.arg[0];
		    eval(n2);
		    i = popsmall(n2->nodetype);
		    if (i <= 0 || not delevent((unsigned int) i)) {
			error("unknown event %ld", i);
		    }
		    n1 = n1->value.arg[1];
	    	}
	    }
...
#ifdef DBMEX
			if (winmode == true) {
				shm_write('S', cursource, 1, curline);
			} else {
				printf("trace:  ");
		        	printlines(curline, curline);
			}
...
#ifdef DBMEX
	if (winmode == true) {
		mylist = buildcmdlist(build(O_PBUFON));
		action = build(O_PRINTSRCPOS, exp);
    		cmdlist_append(action, mylist);
    		cmdlist_append(build(O_PBUFOFF), mylist);
    		e = addevent(event, mylist);
	} else {
#endif
		action = build(O_PRINTSRCPOS, exp);
		if (cond != nil) {
...
Node cond;
{
	...
	if (winmode == true) {
		...
		cmdlist_append(action, mylist);
    		cmdlist_append(build(O_PBUFOFF), mylist);
    		action = build(O_TRACEON, (op == O_TRACEI), mylist);
	} else {
#endif
		action = build(O_PRINTIFCHANGED, exp);
		if (cond != nil) {
		...
		action = build(O_TRACEON, (op == O_TRACEI), 
				buildcmdlist(action));
#ifdef DBMEX
	}
#endif
...
Node p;
{
	...
	if (winmode == true) {
		...
		shm_write('L', cursource, value, l1);
	}
#endif
    printlines(l1, l2);
}
...
public gripe()
{
    ...
    sprintf(subject, "dbx (version %d.%d) gripe", versionNumber,
			subversionNumber);
...
public help()
{
    ...
    puts("stop <at line> if cond - stop <at line> if cond is true");
    puts("cont <signal>          - continue execution with signal");
    puts("step                   - single step one line");
    puts("next                   - step to next line (skip over calls)");
    puts("trace <line#>          - trace execution of the line");
    puts("trace <proc/func>      - trace calls to the procedure");
    ...
    puts("return <proc>          - return to proc or from the current procedure");
    puts("assign var = exp       - assign exp to var");
    puts("up <count>             - move the current context up the stack dount levels");
    puts("down <count>           - move the current context up the stack dount levels");
    puts("whereis identifier     - print out the full qualification of all the symbols whose name matches the given identifier");
    puts("which identifier       - print out the full qualification of name");
/*
...
*/
    ...
    puts("list proc/func         - list the source for proc or func");
    puts("edit <file>            - edit file");
    puts("edit <proc>            - edit the file containing proc");
    puts("file filename          - change the current source file to filename");
    puts("func proc/func         - change the current function");
    puts("use directory-list     - set the list of dir's to be searched when looking for source files");
    puts("alias name(parameters) string - set an alias");
    puts("set name = expression  - define an internal debugger variable");
    puts("tracei address         - trace instruction addres");
    puts("stopi address          - stop at a mchine address");
    puts("address/modifier       - print memory starting at address");
/*
