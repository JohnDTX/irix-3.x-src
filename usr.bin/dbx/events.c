...
static char rcsid[] = "$Header: /d2/3.7/src/usr.bin/dbx/RCS/events.c,v 1.1 89/03/27 17:44:29 root Exp $";
...
Event e;
{
	...
	case O_EQ:
		...
		if (s == linesym) {
		    ...
		    file = srcfilename(codeloc(s));
		    if (file != nil) {
		    	line = srcline(codeloc(s));
		    }
		    if ((file != nil) && (line > 0)) {
		        faddr = objaddr(line, file);
		    	bp = bp_alloc(e, faddr, 0, e->actions);
		    } else if (s->class == PROG) {
		    		bp = bp_alloc(e, codeloc(s), 0, e->actions);
		    } else {
			short ibuff;

			iread(&ibuff, codeloc(s)-4, 2);
			if (ibuff == 0x4e56) {
		    		bp = bp_alloc(e, codeloc(s), 0, e->actions);
			} else {
		    		bp = bp_alloc(e, codeloc(s)-4, 0, e->actions);
			}
		    }
...
Node cond;
{
	...
	if (s == procsym) {
		...
		if (winmode == true) {
			...
			file = srcfilename(func->symvalue.funcv.beginaddr);
			shm_write('B', file, id,
				srcline(func->symvalue.funcv.beinaddr));
		}
#endif
		printf(" in ");
		...
		if (winmode == true) {
	    		...
	    		linenode = place->value.arg[1];
	    		filenode = place->value.arg[0];

			shm_write('B', filenode->value.scon, id, 
				linenode->value.lcon);
		}
#endif
...
int	id;
{
	...
	if (s == procsym) {
		...
		file = srcfilename(func->symvalue.funcv.beginaddr);
		shm_write('B', file, id, 
			srcline(func->symvalue.funcv.beginaddr));
	    }
	} else if (s == linesym) {
	   	Node	filenode;
	   	Node	linenode;
...
Node p;
{
    ...
    if (isnew) {
	if (tracebpts) {
	    printf("findtrinfo:adding trinfo for \"");
	    prtree(stdout, p);
	    printf("\"\n");
	}
	tp = new(Trinfo);
...
Node p;
{
    ...
    tp = findtrinfo(p);
    n = size(p->nodetype);
    if ((p->op == O_SYM) && (p->value.sym->storage == INREG)) {
	val = lval(tp->variable);
	mov(&val, buff, 4);
    } else {
