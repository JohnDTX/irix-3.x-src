...
static char rcsid[] = "$Header: /d2/3.7/src/usr.bin/dbx/RCS/process.c,v 1.1 89/03/27 17:44:49 root Exp $";
...
#endif
#ifdef sgi
#   define regloc(reg)	(ctob(UPAGES)-14-(16*sizeof(Word))+(sizeof(Word)*reg))
#endif
...
int signo;
{
    ...
    if (p->pid == 0) {
	error("pcont:program is not active");
    }
    s = signo;
    do {
#	ifdef sgi
...
char	*str;
{
	...
	find(s,n)
		where s->class == VAR and (s->block == program or
...
#   	if (defined(pmII)||defined(juniper))
		...
		had_trap = 0;
		p->reg[PROGCTR] -= 2;
	    } else if (had_trap and (p->status == STOPPED) and 
			(srcline(p->reg[PROGCTR]) != 0) and
			 (p->reg[PROGCTR] > codestart))  {
		had_trap = 0;
		p->reg[PROGCTR] -= 2;
	    }
#       endif
	...
	addr = (Address) &(((struct user *) 0)->u_signal[p->signo-1]);
	addr = (Address) ptrace(UREAD, p->pid, addr, 0);
	if (addr != 0) {
		addr = sym_addr("sigfunc");
		p->sigstatus = (Address) ptrace(DREAD, p->pid, addr + 
				(p->signo * sizeof(Address)), 0);
	}
...
int nbytes;
{
	...
	if (traceexec) {
		fflush(stdout);
		fprintf(stderr,
		    "\n\tpio: %s of 0x%x bytes procaddr 0x%x dbxaddr 0x%x  ",
			(op == PREAD)?"read":"write", nbytes,
			addr,buff);
		fflush(stderr);
	}
#endif
#ifdef EXPERIMENTAL
    if ((traceexec) && (addr & 1)) dobreak();
#endif

    if (p->status != STOPPED) {
/*
	error("pio:program is not active");
*/
    }
    cp = buff;
    newaddr = addr;
    ...
    while (cp < bufend) {
#ifdef EXPERIMENTAL
	/* GB (7/29/85) If addr was not aligned, addr is now aligned,
	   but buff(cp) is not.  The fetch address is aligned, tho.
	*/
	if (op == PREAD) {
