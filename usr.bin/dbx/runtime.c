...
static char rcsid[] = "$Header: /d2/3.7/src/usr.bin/dbx/RCS/runtime.c,v 1.1 89/03/27 17:44:50 root Exp $";
...
Frame frp;
{
	...
	if (notstarted(process) && (coredump == false)) {
		return(nil);
	}
    newfrp = frp;
...
nextf:
    if (prev_frame + sizeof(struct Frame) <= MAXSTKADDR) {
	dread(&frame, prev_frame, sizeof(struct Frame));
    } else if (MAXSTKADDR - prev_frame > 2 * sizeof(Word)) {
	dread(&frame, prev_frame, MAXSTKADDR - prev_frame);
    } else {
	frame.save_fp = nil;
    }
    if (ntramp == 1) {
...
#ifdef SVS
    if (frame.save_fp == nil && frame.save_pc != (Address) -1) {
	funcp = whatblock(frame.save_pc, true);
	if (funcp->symvalue.funcv.ismain) {
		Symbol	a5_sym;
		if ((a5_sym = lookup(identname("$a5_save"))) != nil) {
			dread(&frame.save_fp, a5_sym->symvalue.offset, 4);
			if (frame.save_fp > MAXSTKADDR) {
				frame.save_fp = nil;
			}
		}
	}
...
#define MOVEML	0x48ee0000	/* moveml #x,sp@ */
...
#ifdef SVS
/*
 * Return the base address for locals for SVS fortran main
 */

public Address main_localbase(frp)
Frame frp;
{
    return (frp == nil) ? reg(MFRP) : frp->save_reg[MFRP];
}
#endif
...
#   endif
#   if (defined(pmII)||defined(juniper))
	base = (frp == nil) ? reg(FRP) : frp->save_fp;
#   endif
    return base;
}
...
{
	long temp;
	temp = f->symvalue.funcv.beginaddr;
    if (isinternal(f)) {
...
#ifdef SVS
		if (f->symvalue.funcv.ismain) {
			f->symvalue.funcv.beginaddr += 16;
		} else {
#endif
