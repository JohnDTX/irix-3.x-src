...
static char rcsid[] = "$Header: /d2/3.7/src/usr.bin/dbx/RCS/scanner.c,v 1.1 89/03/27 17:44:52 root Exp $";
...
public Token yylex()
{
	...
	do {
		if ((firstinput == true) &&
			(s = lookup(identname("main", true))) != nil) {
			firstinput = false;
			strcpy(scanner_linebuf, "func ");
			strcat(scanner_linebuf, "main");
			strcat(scanner_linebuf, "\n");
			line = scanner_linebuf;
		} else {
	    		if (isterm(in)) {
				printf("(%s) ", cmdname);
	    		}
