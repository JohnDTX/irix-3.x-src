...
static char rcsid[] = "$Header: /d2/3.7/src/usr.bin/dbx/RCS/symbols.c,v 1.1 89/03/27 17:44:55 root Exp $";
...
#ifdef SVS
#define isinmain(s)		(s->block->symvalue.funcv.ismain == 1)
#endif
...
Frame frame;
{
	...
	if (isreg(s)) {
		...
		if (isinmain(s)) {
			addr = main_localbase(frp) + s->symvalue.offset;
		} else {
#endif
...
Symbol sym;
{
	...
	case TYPE:
	    /*
	     * This causes problems on the sgi machines 
	     * because of the compiler bug
...
#	    endif
	...
	case SET:
	    ...
	    r = (r + BITS_PER_BYTE - 1) div BITS_PER_BYTE;
	    break;
...
register Symbol s;
{
    ...
    if ((off mod BITS_PER_BYTE) != 0 or (len mod BITS_PER_BYTE) != 0) {
	b = true;
    } else {
	t = rtype(s->type);
	b = (Boolean) (
	    (t->class == SCAL and len != (sizeof(int)*BITS_PER_BYTE)) or
	    len != (size(t)*BITS_PER_BYTE)
	);
    }
    return b;
}
...
#if SVS
			(p1->op == O_SYM) &&
			((p1->value.sym)->type != nil) &&
			((p1->value.sym)->type->class != ARRAY)) {
#endif
		    p->op = O_SYM;
		    p->value.sym = p1->value.sym;
		    dispose(p1);
...
Name fieldname;
{
	...
	if (s == nil) {
		...
		char	c;
		Name	upname;
		p = malloc(strlen(Ident(fieldname)) + 1);
		strcpy(p, Ident(fieldname));
		for (; *p != '\0'; p++) {
			c = *p;
			if (isalpha(c) && islower(c)) {
				*p = toupper(c);
...
Name n;
{
	...
	if (isparamoff(s) && isreg(s->next_sym)) {
		s = s->next_sym;
	}
	printf("[using ");
	printname(stdout, s);
...
Symbol *var_s;
{
    ...
    do {
		if (s->name == n and s->class != FIELD and s->class != TAG) {
			...
			if (f != nil && (s->class != MODULE) &&  isreg(s) && isparamoff(t)) {
				t = s;
				mincount = count;
				b = true;
			}
		    if (f != nil and count < mincount and s->class != MODULE) {
				t = s;
				mincount = count;
				b = true;
		    }
...
Symbol	f;
{
	...
	name = Ident(f->name);
	wrapeename = (char *) malloc(strlen(name) + 2);
	strcpy(wrapeename, name);
	n = identname(wrapeename + 1);
	find (s, (n)) where
		isroutine(s) and (s->symvalue.funcv.haswrapper == 1)
	endfind(s);
	if (s == NULL) {
		wrapeename[0] = '_';
		n = identname(wrapeename);
		find (s, (n)) where
			isroutine(s) and (s->symvalue.funcv.haswrapper == 1)
		endfind(s);
	}
