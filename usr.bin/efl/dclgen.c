...
dclgen()
{
register struct headbits *q; register struct chain *p;
ptr q1;
chainp *y, z;
		...
		if( (p->tag==TNAME || p->tag==TTEMP) &&
			...
			if(first >=0)
				...
				if(q = p->vdim)
					...
					if( (v->tag==TNAME && ((struct varblock *)p)->vclass==CLARG) ||
