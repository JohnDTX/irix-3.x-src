...
static	char	*Sccsid = "@(#)$Header: /d2/3.7/src/usr.bin/RCS/egrep.y,v 1.1 89/03/27 17:40:02 root Exp $";
/*
# $Log:	egrep.y,v $
 * Revision 1.1  89/03/27  17:40:02  root
 * Initial check-in for 3.7
 * 
 * Revision 1.4  84/11/12  17:57:29  bob
 * Fixed to properly compute value for -b flag.
 * 
 */
#include <stdio.h>

#define MAXLIN 2048				/* was 350 */
#define MAXPOS 20000				/* was 4000 */
#define NCHARS 256				/* was 128 */
#define NSTATES 256				/* was 128 */
#define FINAL -1
			...
			do {
				...
				if (c == '-' && cclcnt > 0 && chars[nxtchar-1] != 0) {
					if ((d = nextch()) != 0) {
						c = chars[nxtchar-1];
						while (c < d) {
							if (nxtchar >= MAXLIN) overflo("too many chars");
							chars[nxtchar++] = ++c;
							cclcnt++;
						}
						continue;
...
enter(x) int x; {
	if(line >= MAXLIN) overflo("too many chars");
	name[line] = x;
	left[line] = 0;
	right[line] = 0;
	return(line++);
...
char *why;
{
	fprintf(stderr, "egrep: regular expression too long (%s)\n",why);
	exit(2);
}
	...
	for (s=0; s<=n; s++)  {
		...
		for (c=0; c<NCHARS; c++) {
			if (symbol[c] == 1) { /* nextstate(s,c) */
				...
				if (notin(n)) {
					if (n >= NSTATES) overflo("too many states");
					add(state, ++n);
...
add(array, n) int *array; {
	register i;
	if (nxtpos + count > MAXPOS) overflo("too many positions");
	array[n] = nxtpos;
