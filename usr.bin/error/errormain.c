...
static	char	*Sccsid = "@(#)$Header: /d2/3.7/src/usr.bin/error/RCS/errormain.c,v 1.1 89/03/27 17:49:40 root Exp $";
/*
 * $Log:	errormain.c,v $
 * Revision 1.1  89/03/27  17:49:40  root
 * Initial check-in for 3.7
 * 
 * Revision 1.1  85/03/25  19:19:27  bob
 * Initial revision
 * 
 * Revision 1.2  85/01/30  01:30:39  bob
 * Added sensitivity to $EDITOR env var.
 * 
 */
#include <stdio.h>
#include <ctype.h>
 ...
 *
 *	-v:	after touching all files, overlay $EDITOR, vi(1), ex(1) or ed(1)
	...
	char	**argv;
{
	...
	if (env1) {
		...
		if (env2)
			...
			env2 = env1;
		if (strcmp(env2,"vi") && strcmp(env2,"ex")
		  && strcmp(env2,"edit") && strcmp(env2,"view"))
			try(env1, argc-1, argv+1);
		else
			try(env1, argc, argv);
	}
	try("vi", argc, argv);
