...
function %_atan(arg: real): real;
function %_exp(arg: real): real;
...
function %mys_sncs(x: real; sin_cos_flag: integer): real; 
  const MAX_INPUT = 2147483647.0;  
  var  multiple: longint; xsq: real;
begin 
if IsNum(x) and (abs(x) <= MAX_INPUT)
then begin
     if abs(x) < 0.7854    { This constant may be perturbed } 
     then multiple := 0    { to assure monotonicity } 
     else x := %mypi_reduce(x,multiple);
     xsq := sqr(x);
     multiple := multiple + sin_cos_flag;
     if odd(multiple)
     then begin 
          x := 1.0 + xsq * (-4.99998424e-1 +       { cos }
                     xsq * (+4.16544195e-2  + 
                     xsq * (-1.35794036e-3 ))); 
          multiple := multiple - 1;
          end
     else x := x + x * xsq * (-1.66666502e-1 +     { sin }
                       xsq * (+8.33201640e-3 +
                       xsq * (-1.95018148e-4 ))); 
     if odd(multiple div 2) then x := -x;
     %mys_sncs := x;
     end
else if IsNan(x)
     then %mys_sncs := %gennan4(ord(NOT_A_NUMBER))
     else %mys_sncs := %gennan4(ord(INFEASIBLE));
end; {%mys_sncs}
...
function %_atan{(arg: real): real}; 
  ...
  var x,xsq: real; Reduced: Boolean;
begin
if IsNum(arg)
then begin
     x := abs(arg);
     ...
     if Reduced
     then if arg < 0
          then %_atan := x - PIOVER2
          else %_atan := PIOVER2 - x
     else if arg < 0
          then %_atan := -x
          else %_atan := x;
     end 
else %_atan := %gennan4(ord(NOT_A_NUMBER));
end; {%_atan}

function %_exp{(arg: real): real};
  const MAX_INPUT = +10000.0;
        UPPER_LOG_2_INV = 1.4423828125;
        LOWER_LOG_2_INV = 0.000312228389;
  var multiple: integer;
      p,q: real;
      upper,lower,x,xsq: real;
begin 
if IsNum(arg)
then if abs(arg) < MAX_INPUT
     then begin
          ...
          q := 20.81822806+xsq; 
          %_exp := %myt_pack(0.5 + p/(q-p),multiple + 1);  
          end
     else if arg < 0.0
          then %_exp := %gennan4(ord(UNDERFLOW))      
          else %_exp := %gennan4(ord(OVERFLOW))      
else if IsNan(arg)
     then %_exp := %gennan4(ord(NOT_A_NUMBER))
     else if arg > 0.0
          then %_exp := %gennan4(ord(OVERFLOW))
          else %_exp := %gennan4(ord(UNDERFLOW));
end; {%_exp}

function %_ln{(x: real): real};
  const LOG_2 = 0.693147181; 
  var xsq: real; n: integer;
begin
if IsNum(x)
then if x <= 0.0
     then %_ln := %gennan4(ord(DOMAIN_ERROR))
     else begin
          x := %myt_unpack(x,n); 
          if x > 1.414213
          then begin
          ...
          xsq := sqr(x); 
          %_ln := n*LOG_2 + (x + x) +
                    x*xsq*(1.10445938/(1.65677798 - xsq)); 
          end
else %_ln := %gennan4(ord(NOT_A_NUMBER));
end; {%_ln}
  
function %_sqrt{(x; real): real}; 
  var mantissa,approx: real; exp: integer;
begin
if IsNum(x)
then if x > 0.0
     then begin
          mantissa := %myt_unpack(x,exp); 
          if odd(exp)
          then begin
          ...
          else approx := 0.4173076 * mantissa + 0.5901621; 
          approx := approx + (mantissa/approx);
          approx := approx + (4.0*mantissa/approx);
          %_sqrt := %myt_pack(approx,(exp div 2) - 2);
          end
     else if x = 0.0
          then %_sqrt := 0.0
          else %_sqrt := %gennan4(ord(DOMAIN_ERROR))
else %_sqrt := %gennan4(ord(NOT_A_NUMBER));
end; {%_sqrt}
