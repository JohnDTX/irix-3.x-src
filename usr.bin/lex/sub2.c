/*	@(#)sub2.c	1.3	*/
/* $Source: /d2/3.7/src/usr.bin/lex/RCS/sub2.c,v $ */
static	char	*Sccsid = "@(#)$Revision: 1.1 $";
/* $Date: 89/03/27 18:03:48 $ */
/* sub2.c */
# include "ldefs.c"

extern	int	conehead;		/* rmt 07/84 */
  ...
  int s,c; {
	...
	for(i = 0; i<num; i++){
		...
		|| j == RCCL && member(c,left[curpos])){
			...
			newpos = f+1;
			for(j=0;j<number;j++) {
				if (*newpos < 0 || *newpos > conehead) {
					fflush(stdout);
					fprintf(stderr,"lex: fatal error\n");
					exit(-1);
				}
				temp[*newpos++] = 2;
				}
