...
static char sccsid[] = "@(#)ptx.c	1.1";
/* $Source: /d2/3.7/src/usr.bin/RCS/ptx.c,v $ */
static	char	*Sccsid = "@(#)$Revision: 1.1 $";
/* $Date: 89/03/27 17:40:32 $ */
...
#define TILDE 0177
#define SORT1 "/bin/sort"
#define SORT2 "/usr/bin/sort"
#define	N 30
#define	MAX	N*BUFSIZ
#define LMAX	200
#define MAXT	2048
...
*/
	...
	case 0:		/* child */
		if (foldf == 0) {
			execl(SORT1, SORT1, "-d", sortfile, "-o", sortfile, 0);
			execl(SORT2, SORT2, "-d", sortfile, "-o", sortfile, 0);
		} else {
			execl(SORT1, SORT1, "-df", sortfile, "-o", sortfile, 0);
			execl(SORT2, SORT2, "-df", sortfile, "-o", sortfile, 0);
		}
		fprintf(stderr,"ptx: can't exec sort\n");
		fflush(stderr);
		exit(-1);

	default:	/* parent */
		while((brainless=wait(&status)) != -1 && brainless != pid)
			;
		if (status & 0xffff)
			diag("ptx: sort failed","");
	}
