 ...
 */
 static char rcsid[]=
 "$Header: /d2/3.7/src/usr.bin/rcs/src/rcs/RCS/ci.c,v 1.1 89/03/27 18:21:35 root Exp $ Purdue CS";
/*******************************************************************
...
int usemodtime;			      /* use file modify time instead of cur*/
...
char * argv[];
{
	...
	struct stat workfilestat;/* used for getting the mod file	    */
	...
	catchints();
        cmdid = "ci";
        cmdusage = "command format:\nci -r[rev] -l[rev] -u[rev] -f[rev] -k[rev] -q[rev] -M[rev] -mmsg -nname -Nname -sstate -t[txtfile] file ...";
...
/* file only hack, review of <sys/stat.h>, there will be a quiz...
 * 
 ...
 *
 * avoid imitations, dirs, devs, fifos, symlinks...
 */

	stat(workfilename, &workfilestat);
	if((workfilestat.st_mode & S_IFMT) == S_IFDIR ||
	   (workfilestat.st_mode & S_IFMT) == S_IFCHR ||
	   (workfilestat.st_mode & S_IFMT) == S_IFBLK ||
	   (workfilestat.st_mode & S_IFMT) == S_IFIFO ||
	   (workfilestat.st_mode & S_IFMT) == S_IFLNK) {

        	error("working file %s not an ordinary file",
                	workfilename);
        	continue;
        }
/* end, don't mix with previous filestatus code, kiss */

	stat(workfilename, &workfilestat);
        if (initflag || rcsinitflag) /* get mode for RCSfile from workfile*/
	    filestatus = workfilestat;
        ...
        if (initflag || rcsinitflag ) {
                ...
                if (cmpnum(newdelnum,olddeltanum) <= 0) {
                        error("deltanumber %s too low; must be higher than %s for %s",
                              newdelnum,Head->num, workfilename);
                        continue;
                }
		...
		if (usemodtime)
			newdelta.date =getdate(curdate,true,(long)workfilestat.st_mtime);
		else   
              	newdelta.date  =getdate(curdate,false,0L);
        } else {
        ...
        if (lockflag && !addlock(&newdelta,caller,workfilename)) continue;
...
#       endif
		...
		if (usemodtime)
			fixtime(workfilename,workfilestat.st_atime,
			workfilestat.st_mtime);
		catchints();
        }
        diagnose("done");
 ...
 */
{
        ...
        while (next!=nil) {
                ...
                if (num==nil) {
                        ...
                        if ((whomatch!=0)&&(nummatch==0)) {
                            error("revision %s of %s locked by %s",num, 
				    workfilename, next->login);
                            return false;
                        }
                }
                trail=next;
        ...
        if (next!=nil) {
                ...
                if (!owner) {
                        if (num==nil) error("no lock for %s set by %s",
				workfilename, who);
                        else          error("no lock for %s set by %s for revision %s",
				workfilename, who,num);
                        return nil;
                } elsif (num!=nil) {
                        return delta;
                        ...
                        while (next!=nil) {
                                if (strcmp(num,next->delta->num)==0) {
                                        error("revision %s of %s locked by %s",
						num, workfilename, next->login);
                                        return false;
                                }
                                trail=next;
                                next=next->nextlock;
...
char * getdate(buffer,usemodtime,filemodtime)
char * buffer;
int usemodtime;
long filemodtime;
/* Function: puts the current date, or filemodtime if usemodtime is true,
 * in the form YY.MM.DD.hh.mm.ss\0 into buffer and returns a pointer to it.
 */
{
#       include "time.h"
        ...
        long clock;
        struct tm * tm;
        clock=(usemodtime) ? filemodtime : time(0);
        tm=localtime(&clock);
