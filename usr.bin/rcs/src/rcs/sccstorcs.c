static char *RCSid =
"$Header: /d2/3.7/src/usr.bin/rcs/src/rcs/RCS/sccstorcs.c,v 1.1 89/03/27 18:21:57 root Exp $";
 ...
 *
 * $Log:	sccstorcs.c,v $
 * Revision 1.1  89/03/27  18:21:57  root
 * Initial check-in for 3.7
 * 
 * Revision 1.1  87/01/13  20:32:53  vjs
 * Initial revision
 * 
...
#ifdef sgi
		if (delta->commentary)
			description = concat(description, delta->commentary);
#endif
	}
    }
...
char *description, *rcsfile;
{
    ...
    if (description)
	    (void)tmpnam(&tnm[0]);
    sprintf (command, "%s -i -U -t%s %s", RCS,
	     (description ? tnm : "/dev/null"),
	     rcsfile);
#else
...
#ifdef sgi
    if (description) {
	    register FILE *tfd = fopen(&tnm[0], "w");
	    if (!tfd)
		    return (-1);
	    if (0 > fprintf(tfd, "%s", description)) {
		    (void)unlink(&tnm[0]);
		    return (-1);
	    }
	    (void)fclose(tfd);
