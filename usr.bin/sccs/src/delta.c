...
static	char	*sccsid = "@(#)delta.c	5.16";
/* $Source: /d2/3.7/src/usr.bin/sccs/src/RCS/delta.c,v $ */
static	char	*Sccsid = "@(#)$Revision: 1.1 $";
/* $Date: 89/03/27 18:24:44 $ */
...
char	Diffpgm[]   =   "/usr/bin/bdiff";	/* tries /bin/bdiff & bdiff */
FILE	*Diffin;
FILE	*Gin;
int	Debug = 0;
struct packet gpkt;
		...
		if (status) {		/* diff failed */
			...
			*/
			if (((status >> 8) & 0377) == 32) { /* 'execl' failed */
				sprintf(Error,
					"cannot execute '%s' (de12)",Diffpgm+8);
				fatal(Error);
			}
			/*
			Re-try.
...
int difflim;
{
	...
	if ((i = fork()) < 0) {
		...
		execl(Diffpgm+4,Diffpgm+4,oldf,newf,num,"-s",0);
		execl(Diffpgm+8,Diffpgm+8,oldf,newf,num,"-s",0);
		close(1);
