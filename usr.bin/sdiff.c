...
static char sccsid[] = "@(#)sdiff.c	1.2";
/* $Source: /d2/3.7/src/usr.bin/RCS/sdiff.c,v $ */
static	char	*Sccsid = "@(#)$Revision: 1.1 $";
/* $Date: 89/03/27 17:40:36 $ */
...
char *file;
{
	...
	case 0:
		prog = getenv("EDITOR");
		if (prog != 0 && *prog != '\0')
			execl(prog,  prog, file, 0);
		execl("/bin/vi",     "vi", file, 0);
		execl("/usr/bin/vi", "vi", file, 0);
		execl("/bin/ed",     "ed", file, 0);
	}

	oldintr = signal(SIGINT, SIG_IGN);	/*ignore interrupts while in ed */
	while(pid != (j=wait(&i)) && j != -1)
		;
