...
# $Source: /d2/3.7/src/usr.bin/RCS/uupick.sh,v $
# @(#)$Revision: 1.1 $
# $Date: 89/03/27 17:40:55 $
cdir=`pwd`
trap "exit" 1 2 13 15
# get options
while test $# -gt 1
			  ...
			  then
				...
				case $cmd in
				 ...
				 m)	if test $dir
					...
					find $j -print | cpio -pdmul $tdir 2>&1 | sed '/Cannot chmod/d'
					if test $? -eq 0
					then rm -fr $j
					fi
					break ;;
				 a)	if test $dir
					...
					find * -print | cpio -pdmul $tdir 2>&1 | sed '/Cannot chmod/d'
					if test $? -eq 0
					then rm -fr *
					fi
					break 2 ;;
