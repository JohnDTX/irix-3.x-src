...
struct	proc *aproc;
#ifdef	sgi
long	kavenrun[3];		/* kernel keeps avenrun in integers */
int	hz;
#endif
...
#ifdef	sgi
	lseek(kmem, (long)nl[X_HZ].n_value, 0);
...
#else
#define fxtod(k, i)	(((double)k[i])/1024.0)
		lseek(kmem, (long)nl[X_AVENRUN].n_value, 0);
		read(kmem, kavenrun, sizeof(kavenrun));
		avenrun[0] = fxtod(kavenrun, 0);
		avenrun[1] = fxtod(kavenrun, 1);
		avenrun[2] = fxtod(kavenrun, 2);
#endif
...
#else
		pr[np].w_time = (up.u_utime + up.u_stime) / hz;
		pr[np].w_ctime = (up.u_cutime + up.u_cstime) / hz;
#endif
...
{
	int c, addr, nbad;
	static int abuf[NBPG/sizeof(int)];
	...
	if ((p->w_flag & SLOAD) == 0) {
		...
		if (read(mem,pagetbl,NBPG) != NBPG)
			return(p->w_comm);
		if (pagetbl[NPTEPG-1-HIGHPAGES].pg_fod==0 && pagetbl[NPTEPG-1-HIGHPAGES].pg_pfnum) {
			lseek(mem,ctob(pagetbl[NPTEPG-1-HIGHPAGES].pg_pfnum),0);
