...
#define XINDSIZ XBLKSIZ/sizeof (int)
...
/****/
		printf("%s\n", dash);
		printf(" for extra large file system (2048bytes/block)\n");
		printf(" allocated sectors :     %d\n", xblknew);
		printf(" free sectors :          %d\n", f_blks-xblknew);
/****/
		if(f_blks < blknew) 
...
int s;
{
	...
	if(s == 4) {	/* 2048 byte/block file system */
		blksiz = XBLKSIZ;
		indirt = XINDSIZ;
	}
