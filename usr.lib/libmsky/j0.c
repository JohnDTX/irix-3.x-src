...
long float arg;
{
	long float argsq, n, d;
	long float _lsin(), _lcos(), _lsqrt();
	int i;
	...
	if (arg > XMAX) {
		_fperror.val.dval = (_lsqrt(tpi/arg)*pzero);
		return(_lraise_fperror(MATH,PARTIAL_SLOSS));
	}
	if(arg > 8.){
		asympt(arg);
		n = arg - pio4;
		return(_lsqrt(tpi/arg)*(pzero*_lcos(n) - qzero*_lsin(n)));
		}
	argsq = arg*arg;
...
{
	long float argsq, n, d;
	long float _lsin(), _lcos(), _lsqrt(), _llog(), j0();
	int i;
	...
	if(arg > 8.){
		asympt(arg);
		n = arg - pio4;
		return(_lsqrt(tpi/arg)*(pzero*_lsin(n) + qzero*_lcos(n)));
		}
	...
	return(n/d + tpi*j0(arg)*_llog(arg));
}
